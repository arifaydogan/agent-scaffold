...
Take every failure reason from step 2 and spawn one sub-agent per reason, all in parallel. Each agent takes its assigned failure reason and goes deep on it independently. This is the part that makes a premortem more than a quick opinion: every failure mode gets genuinely investigated, not waved at.

If the surface you're running on cannot spawn sub-agents, do each deep-dive inline instead, one failure reason at a time. Same depth, just sequential.
...
### step 5: deliver the premortem in chat

Write the premortem straight into the chat as one compact, scannable markdown block. No HTML, no report file, no build step. The result lives in the conversation so the user can read it in under a minute, push back, ask for more depth on one failure, or re-run it as the plan changes.
...
It's six months out. This plan failed. Here's the autopsy.

**Most likely failure** — <1-2 sentences>
**Most dangerous failure** — <1-2 sentences>
**Hidden assumption** — *<the one thing taken for granted that they probably never questioned>*

### Failure modes
1. **<title>** *(most likely)* — <one line: how it died and the assumption behind it> · *Watch for:* <one early warning sign>
2. **<title>** *(most dangerous)* — <one line> · *Watch for:* <signal>
3. **<title>** — <one line> · *Watch for:* <signal>
   <one entry per failure reason from step 2, in the same order>

### Revised plan
1. <concrete change mapped to a failure mode — doable this week>
2. <concrete change>

### Before you commit
- [ ] <check that prevents or detects a failure mode>
- [ ] <check>
```

Rules for the chat output:

- **Lead with the synthesis.** Someone scanning for 30 seconds should be able to name the most likely failure, the most dangerous failure, and the top revision.
- **Flag inline.** Mark the most likely and most dangerous modes with `*(most likely)*` / `*(most dangerous)*`. Leave the rest unflagged.
- **One failure mode per reason** from step 2, same order, one line each plus a signal to watch for.
- **Every revision maps to a failure mode.** No generic advice ("consider your pricing"). Concrete and doable this week.
- **Keep it short.** The depth lives in the analysis; the chat output is the distilled, actionable version. Don't paste the full failure stories unless the user asks to expand one.
- **No emoji. No tables.** Plain markdown so it renders cleanly in any chat or terminal.

If — and only if — the user explicitly asks to save it, write the same block to a single `premortem-[slug].md` file. Default is chat only. Never generate HTML.

---

## output format

The premortem lives in the chat: one compact markdown block (see step 5). Synthesis at the top — most likely failure, most dangerous failure, hidden assumption — then the failure modes, the revised plan, and a short pre-launch checklist.

No files by default. No HTML. No server. If the user explicitly wants a copy, save the same block as a single `premortem-[slug].md`.

The synthesis is the product. Make it specific enough to act on this week.
...
```markdown
## Premortem: $297 live workshop on Claude Cowork for marketing teams, 50 seats

It's six months out. This plan failed. Here's the autopsy.

**Most likely failure** — Marketing managers at 10-50 person companies need sign-off to spend $297, and that approval friction quietly kills conversion before launch day.
**Most dangerous failure** — The people who actually buy are solopreneurs, not team managers, so your testimonials and case studies don't resonate with the audience you need for future cohorts. The mismatch compounds.
**Hidden assumption** — *That "marketing managers at 10-50 person companies" is a reachable audience that self-identifies and gathers somewhere you can reach.*

### Failure modes
1. **Approval friction** *(most likely)* — $297 needs a yes from someone else; the buyer stalls. · *Watch for:* checkout starts that don't complete, "let me check with my manager" replies.
2. **Buyer/attendee mismatch** *(most dangerous)* — solopreneurs buy, managers don't; content lands wrong. · *Watch for:* signup job titles skewing solo/founder.
3. **AI-relevance gap** — audience still unsure AI applies to them; the tool-specific pitch is a step too early. · *Watch for:* low reply rate to "Claude Cowork" framing vs generic "AI for marketing."
4. **Prep underbudgeted** — multi-seat demo data takes ~5 weeks, you budgeted 2. · *Watch for:* demo env still broken one week out.
5. **Hollow social proof** — solo attendees produce reviews that don't sell future cohorts. · *Watch for:* testimonials that don't mention team use.
6. **Thin economics** — $14,850 ceiling may not beat other uses of the prep time. · *Watch for:* prep hours climbing past the revenue justification.

### Revised plan
1. Run a $47 pilot for 20 people first. Use it to learn whether real buyers are managers or solopreneurs before building the full thing.
2. Build the full workshop for whoever actually shows up to the pilot, not who you assumed.
3. Lead messaging with the outcome ("ship marketing faster"), not the tool, until the AI-relevance gap closes.

### Before you commit
- [ ] Pilot run and buyer job titles recorded.
- [ ] Demo environment with realistic multi-seat data working end to end.
- [ ] Revenue target checked against actual prep hours.
```
...
- **Always spawn all failure agents in parallel.** Sequential spawning wastes time and lets earlier responses influence later ones. (Only fall back to inline/sequential if the surface can't spawn sub-agents.)
...
- **The synthesis is the product.** Most users read the synthesis and skim the failure modes. Make the synthesis specific and actionable.
- **Keep the chat output short.** The depth happens in the deep-dive agents; what reaches the chat is the distilled version. Don't dump full failure stories unless the user asks to expand one.
