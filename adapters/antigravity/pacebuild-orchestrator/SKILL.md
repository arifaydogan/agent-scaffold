...
description: >
  Orchestrate PaceBuild Jira work end to end in Antigravity. Always use when
  the user provides a PACE issue key, asks to work on a Jira task, or requests
  phase-based delivery. Enforces repository routing, read-only planning phases,
  explicit approval gates, implementation, review, and handoff.
triggers:
  - "PACE-[0-9]+"
  - "Jira task"
  - "orchestration"
...
The user should only need to provide a Jira issue key such as `PACE-43`.
...
Never transition an issue to Done, modify an epic, or merge a pull request.
