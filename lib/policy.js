...
export function computePlanFingerprint(plan) {
  ...
  const parentKey = plan.parentKey || plan.epicKey || plan.configSnapshot?.parentKey || plan.configSnapshot?.epicKey;
  const graphFingerprint = plan.graphFingerprint || plan.configSnapshot?.graphFingerprint;
  const childBaseSha = plan.childBaseSha || plan.configSnapshot?.childBaseSha;
  const baseSha = plan.baseSha || plan.configSnapshot?.baseSha;
  ...
  const executionCandidates = Array.isArray(plan.executionCandidates) && plan.executionCandidates.length > 0
    ? plan.executionCandidates
    : (Array.isArray(plan.configSnapshot?.executionCandidates) ? plan.configSnapshot.executionCandidates : []);
  const approvedExecution = executionCandidates[0] || plan.execution || {};
  const canonicalExecutionCandidates = executionCandidates.map(candidate => ({
  ...
  const canonical = {
    ...
    repository: plan.projectProfile?.repository || null,
    ...
    executorProvider: approvedExecution.provider || plan.configSnapshot?.executorProvider || null,
    executorModel: approvedExecution.model || plan.configSnapshot?.executorModel || null,
    executorModelProfile: approvedExecution.modelProfile || plan.configSnapshot?.executorModelProfile || null,
    executorEffort: approvedExecution.effort || plan.configSnapshot?.executorEffort || null,
    executionCandidates: canonicalExecutionCandidates,
    adaptiveRouting: plan.adaptiveRouting || plan.configSnapshot?.adaptiveRouting || null,
    ...
    ...(parentKey ? { parentKey: String(parentKey) } : {}),
    ...(graphFingerprint ? { graphFingerprint: String(graphFingerprint) } : {}),
    ...(childBaseSha ? { childBaseSha: String(childBaseSha).toLowerCase() } : {}),
    ...(baseSha ? { baseSha: String(baseSha).toLowerCase() } : {})
  };
...
export function resolveOperatingMode(settings = {}) {
  ...
  const raw = settings?.data?.operatingMode ?? projMode ?? polMode ?? settings?.operatingMode;
