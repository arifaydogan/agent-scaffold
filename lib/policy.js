...
export function evaluateIssue(issue, policy, options = {}) {
  ...
  if (options.settings) {
    ...
    // Agent Registry Lifecycle Check: disabled or archived agents cannot receive new work
    if (options.store && typeof options.store.getAgentDefinition === "function" && options.plan) {
      const agentId = options.plan.taskAgent || options.plan.persona;
      if (agentId) {
        const agentRecord = options.store.getAgentDefinition(agentId);
        if (agentRecord) {
          if (agentRecord.status === "disabled") {
