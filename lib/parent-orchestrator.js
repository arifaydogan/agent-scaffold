...
function resolveParentReviewerSnapshot(settings, store, parent) {
  const reviewPolicy = settings.data?.policy?.review || {};
  const taskAgent = reviewPolicy.taskAgent || reviewPolicy.reviewer || "correctness-reviewer";
  const reviewerDef = typeof store.getAgentDefinition === "function"
    ? store.getAgentDefinition(taskAgent)
    : null;
  const agentExecutor = reviewerDef?.definition?.executor || {};
  const requestedProfile = {
    reviewProvider: reviewPolicy.provider || agentExecutor.provider || null,
    reviewModel: reviewPolicy.model || agentExecutor.model || null,
    reviewModelProfile: reviewPolicy.modelProfile || agentExecutor.modelProfile || "medium",
    reviewTaskAgent: taskAgent,
    reviewPersona: reviewPolicy.persona || taskAgent,
    reviewEffort: reviewPolicy.effort || "medium"
  };

  let profile = null;
  try {
    profile = selectReviewProfile(
      settings,
      { key: parent.key || parent.parentKey, summary: parent.summary, canonicalState: "review" },
      { reviewer: taskAgent },
      requestedProfile
    );
  } catch {}
  ...
  return {
    reviewerAgentId: reviewerDef?.id || taskAgent,
    reviewerVersion: reviewerDef?.version ?? null,
    reviewerHash: reviewerDef?.definitionHash || null,
    persona: profile?.persona || requestedProfile.reviewPersona,
    taskAgent,
    provider: profile?.provider || requestedProfile.reviewProvider,
    model: profile?.model || requestedProfile.reviewModel,
    modelProfile: profile?.modelProfile || requestedProfile.reviewModelProfile,
    effort: profile?.effort || requestedProfile.reviewEffort
  };
}
...
export async function discoverAndPinParent(settings, store, parent, options = {}) {
  ...
  const reviewerSnapshot = existing?.reviewerSnapshot || resolveParentReviewerSnapshot(settings, store, parent);
...
export async function runParentIntegrationReview(settings, store, parentKey, options = {}) {
  ...
  const parentHadPinnedReviewer = Boolean(parent.reviewerSnapshot);
  let pinnedSnapshot = parent.reviewerSnapshot || null;
  if (!pinnedSnapshot) {
    pinnedSnapshot = resolveParentReviewerSnapshot(settings, store, parent);
  }

  const reviewerAgentId = pinnedSnapshot.reviewerAgentId || pinnedSnapshot.taskAgent;
  const reviewerVersion = pinnedSnapshot.reviewerVersion;
  const reviewerHash = pinnedSnapshot.reviewerHash || null;
  const explicitReviewer = pinnedSnapshot.taskAgent || reviewerAgentId;
  const liveReviewerDef = typeof store.getAgentDefinition === "function"
    ? store.getAgentDefinition(reviewerAgentId)
    : null;
  const reviewerDef = typeof store.getAgentDefinition === "function" && reviewerVersion != null
    ? store.getAgentDefinition(reviewerAgentId, reviewerVersion)
    : null;

  let reviewerFailure = null;
  if (!liveReviewerDef) {
    reviewerFailure = `Reviewer agent '${reviewerAgentId}' is not registered in agent registry`;
  } else if (liveReviewerDef.status !== "enabled") {
    reviewerFailure = `Reviewer agent '${reviewerAgentId}' is disabled in agent registry`;
  } else if (!reviewerDef) {
    reviewerFailure = `Pinned reviewer version '${reviewerAgentId}@${reviewerVersion}' is unavailable`;
  } else if (reviewerHash && reviewerDef.definitionHash !== reviewerHash) {
    reviewerFailure = `Pinned reviewer definition hash mismatch for '${reviewerAgentId}@${reviewerVersion}'`;
  }

  if (reviewerFailure) {
    store.updateParentExecutionState(parentKey, "blocked", { reason: reviewerFailure });
    recordParentTelemetryEvent(store, {
      parentKey,
      event: "parent_blocked",
      stage: "terminal",
      status: "blocked",
      metadata: { reason: reviewerFailure }
    });
    return { ok: false, blocked: true, parentState: "blocked", reason: reviewerFailure };
  }
  ...
  try {
    profile = selectReviewProfile(settings, dummyIssue, { reviewer: explicitReviewer }, {
      reviewProvider: pinnedSnapshot.provider,
      reviewModel: pinnedSnapshot.model,
      reviewModelProfile: pinnedSnapshot.modelProfile,
      reviewTaskAgent: explicitReviewer,
      reviewPersona: pinnedSnapshot.persona || explicitReviewer,
      reviewEffort: pinnedSnapshot.effort
    });
  } catch (err) {
    const reason = `Pinned reviewer configuration cannot execute: ${String(err.message || err).slice(0, 300)}`;
  ...
  if (!profile) {
    const reason = "Pinned reviewer configuration is incomplete";
    store.updateParentExecutionState(parentKey, "blocked", { reason });
    recordParentTelemetryEvent(store, {
      parentKey,
      event: "parent_blocked",
  ...
  const modelProfile = profile.modelProfile;
  ...
  if (!parentHadPinnedReviewer) {
    store.upsertParentExecution({ ...parent, reviewerSnapshot: pinnedSnapshot });
  }
