...
export async function runParentIntegrationReview(settings, store, parentKey, options = {}) {
  ...
  if (options.injectedReviewOutcome) {
    ...
    try {
      ...
      const reviewPrompt = `Aggregate Integration Review for parent ${parentKey}: ${parent.summary}\nBase SHA: ${parentBaseSha}\nHead SHA: ${integrationHeadSha}\nPerform an aggregate review across all integrated changes and output structured review findings.`;
      const built = buildExecutorCommand({ settings, profile, prepared, prompt: reviewPrompt, runId: revRunId });
    ...
    if (typeof store.recordTelemetryEvent === "function") {
      try {
        store.recordTelemetryEvent({
          ...
          status: "failed",
          sequence: 3,
          duration_ms: reviewTelemetry?.durationMs ?? null,
          input_tokens: reviewTelemetry?.usage?.input_tokens ?? null,
          output_tokens: reviewTelemetry?.usage?.output_tokens ?? null,
          usage_available: reviewTelemetry?.usage ? 1 : 0,
          errorCategory: "reviewer_execution_error",
    ...
    if (typeof store.recordTelemetryEvent === "function") {
      try {
        store.recordTelemetryEvent({
          ...
          output_tokens: reviewTelemetry?.usage?.output_tokens ?? null,
          usage_available: reviewTelemetry?.usage ? 1 : 0,
          errorCategory: "reviewer_schema_error",
          errorMessage: reason,
          createdAt: new Date().toISOString()
        });
      } catch {}
