...
export function buildPmWorkspace(settings, options = {}) {
  ...
  for (const disc of discovered) {
    if (hiddenIssueKeys.has(String(disc.issueKey || "").toUpperCase())) continue;
  ...
  // Include parent executions with authoritative approval state
  const parentExecutions = typeof store.listParentExecutions === "function" ? store.listParentExecutions() : [];
  for (const parent of parentExecutions) {
    const parentKey = parent.parentKey;
    if (hiddenIssueKeys.has(String(parentKey || "").toUpperCase())) continue;
    const pmDecisions = typeof store.getPmDecisions === "function" ? store.getPmDecisions(parentKey) : [];
    const pendingRequests = pmDecisions.filter(d => d.type === "approval_requested");
    const latestRequest = pendingRequests.at(-1) || null;
    const reqPayload = latestRequest?.payload || {};
    const matchingDecision = latestRequest ? pmDecisions.findLast(
      d => d.type === "execution_approval" &&
           d.payload?.action === reqPayload.action &&
           d.payload?.planFingerprint === reqPayload.planFingerprint
    ) : null;

    const isUnresolvedApprovalRequested = Boolean(latestRequest && !matchingDecision && parent.state === "waiting_approval");
    const isApproved = matchingDecision?.payload?.approved === true;
    const isRejected = matchingDecision?.payload?.approved === false;

    const existingIndex = items.findIndex(i => i.issueKey === parentKey);

    if (isUnresolvedApprovalRequested) {
      const parentItem = {
        issueKey: parentKey,
        summary: parent.summary || parentKey,
        canonicalState: "awaiting_approval",
        currentRunState: parent.state,
        operationalGroup: "awaitingApproval",
        operatingMode: resolveOperatingMode(settings),
        persona: "parent-orchestrator",
        taskAgent: "parent-orchestrator",
        agentId: "parent-orchestrator",
        agentVersion: 1,
        agentHash: null,
        agentLiveStatus: "enabled",
        agentLiveVersion: 1,
        agentLiveHash: null,
        agentUpToDate: true,
        executorProvider: "builtin",
        executorModel: null,
        executorModelProfile: null,
        risk: "normal",
        reworkAttempt: 0,
        ...
        action: reqPayload.action || "branchCreation",
        blockedReason: null,
        approvalReason: reqPayload.reason || latestRequest?.payload?.reason || `Parent action "${reqPayload.action}" requires PM authorization`,
        planFingerprint: reqPayload.planFingerprint,
        allowedPaths: [],
        rationale: [parent.summary || "Parent execution gate awaiting authorization"],
        sourceProvider: parent.sourceProvider || "jira",
        sourceUrl: parent.sourceUrl || null,
        runId: existingIndex >= 0 ? items[existingIndex].runId : null,
        createdAt: parent.createdAt,
        updatedAt: parent.updatedAt
      };
      if (existingIndex >= 0) {
        items[existingIndex] = parentItem;
      } else {
        seenIssues.add(parentKey);
        items.push(parentItem);
      }
    } else if (parent.state === "waiting_human") {
      const parentItem = {
        issueKey: parentKey,
        summary: parent.summary || parentKey,
        canonicalState: "human_approval",
        currentRunState: parent.state,
        ...
        blockedReason: null,
        approvalReason: null,
        planFingerprint: parent.graphFingerprint,
        allowedPaths: [],
        rationale: ["Parent integration review clean; ready for final human sign-off."],
        sourceProvider: parent.sourceProvider || "jira",
        sourceUrl: parent.sourceUrl || null,
        runId: existingIndex >= 0 ? items[existingIndex].runId : null,
        createdAt: parent.createdAt,
    ...
    } else if (parent.state === "blocked" || isRejected) {
      const blockedReason = matchingDecision?.payload?.reason || parent.completionPacket?.staleReason || "Parent execution blocked";
      const parentItem = {
        issueKey: parentKey,
        summary: parent.summary || parentKey,
        canonicalState: "blocked",
        currentRunState: parent.state,
        operationalGroup: "blocked",
        operatingMode: resolveOperatingMode(settings),
        persona: "parent-orchestrator",
        taskAgent: "parent-orchestrator",
        agentId: "parent-orchestrator",
        ...
        maxReworkAttempts: 3,
        humanActionRequired: true,
        action: reqPayload.action || "orchestration",
        blockedReason,
        approvalReason: null,
        planFingerprint: reqPayload.planFingerprint || parent.graphFingerprint,
        allowedPaths: [],
    ...
    } else if (isApproved && existingIndex >= 0 && items[existingIndex].operationalGroup === "awaitingApproval") {
      items[existingIndex].operationalGroup = "executing";
      items[existingIndex].canonicalState = "executing";
      items[existingIndex].humanActionRequired = false;
    }
  }
...
export function buildPmWorkItemDetail(settings, issueKey, options = {}) {
  ...
  const parent = typeof store.getParentExecution === "function" ? store.getParentExecution(targetKey) : null;
  if (parent) {
    const pmDecisions = store.getPmDecisions(targetKey);
    const pendingRequests = pmDecisions.filter(d => d.type === "approval_requested");
    const latestRequest = pendingRequests.at(-1) || null;
    const reqPayload = latestRequest?.payload || {};
    const matchingApproval = latestRequest ? pmDecisions.findLast(
      d => d.type === "execution_approval" &&
           d.payload?.action === reqPayload.action &&
           d.payload?.planFingerprint === reqPayload.planFingerprint
    ) : null;

    const isAwaitingApproval = parent.state === "waiting_approval" || (latestRequest && !matchingApproval);
    const canApprove = Boolean(isAwaitingApproval);
    const currentAction = reqPayload.action || (parent.state === "waiting_human" ? "finalMerge" : "branchCreation");
    const currentFingerprint = reqPayload.planFingerprint || parent.graphFingerprint;
    const isApproved = matchingApproval?.payload?.approved === true;
    const isRejected = matchingApproval?.payload?.approved === false;
    const approvalState = isApproved ? "approved" : (isRejected ? "rejected" : (isAwaitingApproval ? "pending" : "not_required"));
    ...
    return {
      ...
      workItem: {
        ...
        id: null,
        summary: parent.summary || targetKey,
        sourceProvider: parent.sourceProvider || "jira",
        sourceId: parent.sourceId || targetKey,
        sourceUrl: parent.sourceUrl || null,
        canonicalState: parent.state === "waiting_human" ? "human_approval" : (isAwaitingApproval ? "awaiting_approval" : (parent.state === "blocked" ? "blocked" : "executing")),
        status: parent.state,
        operationalGroup: isAwaitingApproval ? "awaitingApproval" : (parent.state === "waiting_human" ? "humanApproval" : (parent.state === "blocked" ? "blocked" : "executing")),
        labels: []
      },
      orchestratorDecision: {
        orchestratorProvider: "builtin",
        persona: "parent-orchestrator",
        taskAgent: "parent-orchestrator",
        skills: [],
        capabilities: [],
        ...
        dependencies: [],
        allowedPaths: [],
        rationale: [parent.summary || "Parent execution in progress"],
        stableMetadata: {},
      ...
      execution: {
        ...
        worktree: parent.integrationWorktree,
        commit: parent.baseSha,
        attempt: 0,
        maxAttempts: 0,
        workerPid: null,
      ...
      review: (() => {
        const intRev = parent.completionPacket?.integrationReview || null;
        return {
          reviewerTaskAgent: intRev?.reviewerAgentId || intRev?.reviewer || null,
          reviewAgentVersion: intRev?.reviewerVersion ?? (intRev?.agentVersion ?? null),
          reviewAgentHash: intRev?.reviewerHash || intRev?.agentHash || null,
          reviewProvider: intRev?.provider || null,
          reviewModel: intRev?.model || null,
          reviewModelProfile: intRev?.modelProfile || null,
          verdict: intRev?.verdict || null,
          latestImplementationSha: parent.completionPacket?.integrationHeadSha || null,
          structuredFindings: intRev?.findings || intRev?.structuredFindings || [],
          reviewCycles: []
        };
      })(),
      humanControl: {
        currentRequiredHumanAction: isAwaitingApproval ? "approval_needed" : (parent.state === "waiting_human" ? "human_approval_gate" : null),
        humanActionRequired: isAwaitingApproval || parent.state === "waiting_human" || parent.state === "blocked",
        operatingMode: resolveOperatingMode(settings),
      ...
      blockedInfo: {
        isBlocked: parent.state === "blocked",
        reason: parent.state === "blocked" ? (parent.reason || "Blocked") : null,
        originatingAction: currentAction,
        taskAgent: "parent-orchestrator",
        reviewer: null,
        attempt: 0,
      ...
      history: pmDecisions.map(d => ({
        id: `dec-${d.id}`,
        timestamp: d.createdAt,
        stage: d.type,
        label: `Decision: ${d.type}`,
        actor: { type: "pm", id: "pm-user" },
        details: d.payload
      }))
    };
  }
  ...
  const progressEvents = latestRun.events.filter(event =>
    event.state === "progress" && typeof event.payload?.text === "string" && event.payload.text.trim()
  );
  const recentProgress = progressEvents.slice(-5).map(event => ({
    text: event.payload.text.slice(0, 1200),
  ...
  const blockedDetail = isBlocked
    ? [latestPayload.error, latestPayload.detail]
  ...
  return {
    ...
    execution: {
      ...
      currentActivity: latestProgress?.text || null,
      lastActivityAt: latestProgress?.timestamp || latestEvent.created_at || latestRun.updated_at,
      recentProgress,
      ...
      adaptiveRoutingEnabled: (plan.adaptiveRouting || snapshot.adaptiveRouting)?.enabled === true,
      candidates: Array.isArray(plan.executionCandidates)
        ? plan.executionCandidates.map(candidate => ({
            provider: candidate.provider || null,
            model: candidate.model || null,
            modelProfile: candidate.modelProfile || null,
            effort: candidate.effort || null
          }))
        : [],
      failoverContext: plan.failoverContext || null
    },
    review: {
    ...
    blockedInfo: {
      ...
      failureCategory: latestPayload.failure?.category || null,
      autoFailoverEligible: latestPayload.failoverEligible === true,
...
export function handlePmApproval(settings, issueKey, {
  ...
  const executeApproval = () => {
    ...
    if (latestRequest) {
      const reqPayload = latestRequest.payload || {};
      const matchingApproval = pmDecisions.findLast(
        d => d.type === "execution_approval" &&
             d.payload?.action === reqPayload.action &&
             d.payload?.planFingerprint === reqPayload.planFingerprint
      );
      ...
      if (matchingApproval) {
        if (matchingApproval.payload?.approved) {
          const error = new Error(`Action "${reqPayload.action}" is already approved; no approval pending.`);
          error.statusCode = 409;
          throw error;
        } else {
          const error = new Error(`Action "${reqPayload.action}" is already rejected; no approval pending.`);
          error.statusCode = 409;
          throw error;
        }
      }

      pendingAction = reqPayload.action;
      pendingAttempt = Number(reqPayload.attempt || 0);
      currentFingerprint = reqPayload.planFingerprint;
      plan = reqPayload.plan || {};
    } else {
      // Check normal run
      ...
      if (!runRow) {
        if (parent) {
          const error = new Error(`Action "${action}" does not require approval; no approval pending.`);
          error.statusCode = 409;
          throw error;
        }
      ...
      plan = latestRun.payload || {};
      currentFingerprint = computePlanFingerprint(plan);

      const approvalCheck = checkApprovalNeeded(latestRun, settings, store);
      pendingAction = approvalCheck.action;
      pendingAttempt = approvalCheck.attempt;
    ...
    // Validate client parameters against authoritative pending gate
  ...
  if (typeof store.withTransaction === "function") {
    return store.withTransaction(executeApproval);
  }
...
export function handlePmRejection(settings, issueKey, {
  ...
  if (typeof store.withTransaction === "function") {
    return store.withTransaction(executeRejection);
  }
  return executeRejection();
}

export const getPmDetail = buildPmWorkItemDetail;
