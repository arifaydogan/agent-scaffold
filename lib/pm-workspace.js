...
export function handlePmApproval(settings, issueKey, {
  ...
  // Resolve actual pending action and attempt from durable runtime context
  const pendingAction = determineItemAction(latestRun, settings, store);
  const pendingAttempt = determineItemAttempt(latestRun);
  ...
  if (attempt !== undefined && attempt !== null) {
    ...
    if (pendingAction === "rework" && numericAttempt !== pendingAttempt) {
