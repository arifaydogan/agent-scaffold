...
export class GraftCodeIntelligenceProvider extends CodeIntelligenceProvider {
  ...
  async detectChanges(request = {}) {
    ...
    const reasons = [];
    const fileTraceResults = []; // per-file: "success_with_edges" | "success_zero_edges" | "failed"

    let graphIsFresh = false;

    if (client.toolSchemas.has("graft_check_freshness")) {
      try {
        const freshRaw = await client.callTool("graft_check_freshness", {});
        const freshParsed = typeof freshRaw === "string" ? parseGraftFreshnessText(freshRaw) : { isFresh: freshRaw?.fresh === true };
    ...
    if (callers.length > 5) {
      ...
      // Every required trace succeeded with zero edges against fresh graph
      risk = "low";
      coverage = "covered";
      reasons.push("No indexed inbound callers found in the fresh covered Graft graph");
    } else {
      // One or more traces failed, or graph is not fresh/indexed
      risk = "unknown";
