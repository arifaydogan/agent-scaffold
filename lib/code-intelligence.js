...
function parseGraftRepoMapText(text, defaultProject) {
...
function parseGraftFindCodeText(text, query, limit = 10) {
  ...
  for (let i = 0; i < lines.length && matches.length < limit; i++) {
    ...
    const hitRegex = /(?:^\d+\.\s*)?([a-zA-Z0-9_./\\-]+\.[a-zA-Z0-9]+):(\d+)(?::\s*|\s+`?([a-zA-Z0-9_$]+)`?)?(?:\s*\(score:\s*([\d.]+)\))?/;
    ...
    if (hitMatch) {
      ...
      const file = hitMatch[1];
      const lineNum = Number(hitMatch[2]);
      const symbol = hitMatch[3] || null;
      const score = hitMatch[4] ? Number(hitMatch[4]) : 0.9;
      currentMatch = {
        symbol: symbol || (query && query.match(/^[a-zA-Z0-9_$]+$/) ? query : null),
        kind: "symbol",
        file,
        line: lineNum,
        qualifiedName: file && symbol ? `${file}:${symbol}` : (file ? `${file}:${lineNum}` : null),
        score,
        evidence: line
      };
...
function parseGraftTraceText(text, direction) {
  ...
  for (const line of lines) {
    if (/^(inbound|outbound|calls|no\s+calls|trace)/i.test(line)) continue;

    const callMatch = line.match(/(?:<-\s*|->\s*)?([a-zA-Z0-9_$]+)(?:\s*\(([^:\s)]+):?(\d+)?\))?/);
    if (callMatch && callMatch[1]) {
      const sym = callMatch[1];
      if (sym === "Inbound" || sym === "Outbound" || sym === "Calls" || sym === "Depth") continue;
      const file = callMatch[2] || null;
      const lineNum = callMatch[3] ? Number(callMatch[3]) : null;

      if (line.includes("<-") || direction === "in") {
        callers.push({ symbol: sym, file, line: lineNum });
      } else if (line.includes("->") || direction === "out") {
...
function parseGraftFreshnessText(text) {
  if (typeof text !== "string") {
    return { isIndexed: false, isFresh: false, warning: "Graft freshness response was not text" };
  }

  const lower = text.toLowerCase();
  if (lower.includes("no graph") || lower.includes("not indexed") || lower.includes("unindexed") || lower.includes("run graft index") || lower.includes("no .graft")) {
    return { isIndexed: false, isFresh: false, warning: "Repository has no Graft graph or is not indexed" };
  }
  if (lower.includes("stale") || lower.includes("drift") || lower.includes("out of sync") || lower.includes("modified file")) {
    return { isIndexed: true, isFresh: false, warning: "Graft graph is stale or out of sync with repository" };
  }
  if (lower.includes("up to date") || lower.includes("fresh") || lower.includes("synced") || lower.includes("indexed") || lower.includes("clean") || lower.includes("files")) {
    return { isIndexed: true, isFresh: true, warning: null };
  }
  return { isIndexed: false, isFresh: false, warning: `Graft freshness output could not be interpreted: ${text.slice(0, 100)}` };
}
...
export class GraftCodeIntelligenceProvider extends CodeIntelligenceProvider {
  ...
  _getClient(projectPath) {
    const canonicalRoot = this._canonicalRepo(projectPath);
    const existing = this.clients.get(canonicalRoot);
    if (existing && existing.initialized && existing.toolDiscoveryComplete) {
      return existing;
    }
    if (existing) {
      existing.close();
      this.clients.delete(canonicalRoot);
    }

    const cmd = Array.isArray(this.config.command) && this.config.command.length > 0
      ? this.config.command
      : ["graft", "mcp"];
    const client = new McpStdioClient(cmd[0], cmd.slice(1), {
      cwd: canonicalRoot,
      timeoutMs: this.config.timeoutMs || 10000,
      maxBufferSize: this.config.maxBufferSize,
      env: this.config.env,
    ...
    this.clients.set(canonicalRoot, client);
    return client;
  }
  ...
  async health(repoPath = null) {
    ...
    try {
      ...
      // Check Graft freshness without arguments per upstream contract
      if (client.toolSchemas.has("graft_check_freshness")) {
        try {
          const raw = await client.callTool("graft_check_freshness", {});
          const parsed = typeof raw === "string"
            ? parseGraftFreshnessText(raw)
            : { isIndexed: raw?.fresh !== false && raw?.status !== "unindexed", isFresh: raw?.fresh === true || raw?.status === "fresh", warning: null };
          isIndexed = parsed.isIndexed;
          isFresh = parsed.isFresh;
          warning = parsed.warning;
        } catch (err) {
          isIndexed = false;
          warning = `Graft freshness check failed: ${err.message}`;
        }
      } else {
        warning = "graft_check_freshness tool is not advertised by provider";
      }
      ...
      return {
        ...
        lastIndexedAt: null,
        capabilities: toolNames,
        warning: warning || (isIndexed ? null : "Repository has no Graft graph or is not indexed")
      };
    } catch (err) {
      return {
        provider: this.name,
  ...
  async getArchitecture(options = {}) {
    ...
    const args = options.maxDirs ? { max_dirs: Number(options.maxDirs) } : {};
    const raw = await client.callTool("graft_repo_map", args);
    ...
    return {
      provider: this.name,
      project: raw.project || raw.project_name || projectName,
      generation: raw.generation || raw.version || null,
      languages: Array.isArray(raw.languages) ? raw.languages : [],
      packages: Array.isArray(raw.packages) ? raw.packages : (Array.isArray(raw.modules) ? raw.modules : []),
      entryPoints: Array.isArray(raw.entry_points || raw.entryPoints || raw.hubs) ? (raw.entry_points || raw.entryPoints || raw.hubs) : [],
      routes: Array.isArray(raw.routes) ? raw.routes : [],
      hotspots: Array.isArray(raw.hotspots || raw.central_nodes) ? (raw.hotspots || raw.central_nodes) : [],
      boundaries: Array.isArray(raw.boundaries) ? raw.boundaries : [],
      clusters: Array.isArray(raw.clusters) ? raw.clusters : [],
      evidence: Array.isArray(raw.evidence) ? raw.evidence : (typeof raw.summary === "string" ? [raw.summary] : [])
    };
  }
  ...
  async searchCode(query, options = {}) {
    ...
    let matches = [];
    if (client.toolSchemas.has("graft_find_code")) {
      const raw = await client.callTool("graft_find_code", { query: query.trim(), limit });
      if (typeof raw === "string") {
        matches = parseGraftFindCodeText(raw, query.trim(), limit);
      } else if (Array.isArray(raw?.nodes) || Array.isArray(raw?.results) || Array.isArray(raw?.matches) || Array.isArray(raw)) {
        const items = Array.isArray(raw?.nodes) ? raw.nodes : (Array.isArray(raw?.results) ? raw.results : (Array.isArray(raw?.matches) ? raw.matches : raw));
        matches = items.slice(0, limit).map((m) => {
          const symbol = m.symbol || m.name || m.identifier || null;
          const file = m.file || m.file_path || m.filePath || m.path || null;
          const line = Number(m.line || m.line_number || m.start_line || 1);
          return {
            symbol,
            kind: m.kind || m.type || "symbol",
            file,
            line,
            qualifiedName: m.qualified_name || (file && symbol ? `${file}:${symbol}` : null),
            score: typeof m.score === "number" ? Number(m.score.toFixed(4)) : 1.0,
            evidence: typeof m.evidence === "string" ? m.evidence.slice(0, 300) : (typeof m.snippet === "string" ? m.snippet.slice(0, 300) : null)
          };
        }).filter((m) => m.file || m.symbol);
      }
    }

    if (matches.length === 0 && client.toolSchemas.has("graft_find_all")) {
      try {
        const rawAll = await client.callTool("graft_find_all", { pattern: query.trim() });
  ...
  async tracePath(request = {}) {
    ...
    if (direction === "both" || direction === "in") {
      try {
        const inRaw = await client.callTool("graft_trace_calls", { symbol, direction: "in", depth });
        if (typeof inRaw === "string") {
          const parsed = parseGraftTraceText(inRaw, "in");
          callers = parsed.callers;
        } else if (Array.isArray(inRaw?.callers)) {
          callers = inRaw.callers;
        }
      } catch {}
    }
    ...
    if (direction === "both" || direction === "out") {
      try {
        const outRaw = await client.callTool("graft_trace_calls", { symbol, direction: "out", depth });
        if (typeof outRaw === "string") {
          const parsed = parseGraftTraceText(outRaw, "out");
          callees = parsed.callees;
        } else if (Array.isArray(outRaw?.callees)) {
          callees = outRaw.callees;
        }
      } catch {}
    }
  ...
  async detectChanges(request = {}) {
    ...
    if (client.toolSchemas.has("graft_trace_calls")) {
      for (const file of changedFiles.slice(0, 3)) {
        try {
          const traceRaw = await client.callTool("graft_trace_calls", { symbol: file, direction: "in", depth: 2 });
          const parsed = typeof traceRaw === "string" ? parseGraftTraceText(traceRaw, "in") : traceRaw;
          if (Array.isArray(parsed?.callers)) {
            for (const c of parsed.callers) {
    ...
    const risk = callers.length > 5 ? "high" : (callers.length > 0 ? "medium" : "low");
    if (callers.length > 0) {
      reasons.push(`Discovered ${callers.length} inbound caller(s) through Graft call graph`);
    } else {
      reasons.push(`No inbound callers found for ${changedFiles.length} changed file(s)`);
    }
    ...
    return {
      ...
      risk,
      reasons,
      coverage: (callers.length > 0 || dependents.length > 0) ? "covered" : "unknown"
    };
  }
  ...
  async checkCoverage(request = {}) {
    ...
    if (!client.toolSchemas.has("graft_check_freshness")) {
      return {
        ...
        gaps: [],
        coverageRatio: 0.0,
        warnings: ["Graft check_freshness tool is unavailable"]
      };
    }

    const raw = await client.callTool("graft_check_freshness", {});
    const { isIndexed, isFresh, warning } = typeof raw === "string"
      ? parseGraftFreshnessText(raw)
      : { isIndexed: raw?.fresh !== false && raw?.status !== "unindexed", isFresh: raw?.fresh === true || raw?.status === "fresh", warning: null };
    ...
    if (isFresh) {
      status = "covered";
      coverageRatio = 1.0;
    } else if (isIndexed && !isFresh) {
      status = "partial";
      coverageRatio = 0.5;
      warnings.push(warning || "Graft graph is stale or out of sync with repository");
    } else {
      status = "unknown";
      coverageRatio = 0.0;
      warnings.push(warning || "Repository has no Graft graph or is not indexed");
    }
  ...
  async getSnippet(request = {}) {
    ...
    const raw = await client.callTool("graft_file_api", { file: relFile });
    const content = typeof raw === "string"
      ? raw
      : (typeof raw?.content === "string" ? raw.content : (typeof raw?.api === "string" ? raw.api : ""));
