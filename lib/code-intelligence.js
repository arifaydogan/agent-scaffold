...
export class GraftCodeIntelligenceProvider extends CodeIntelligenceProvider {
  ...
  async health(repoPath = null) {
    ...
    try {
      ...
      // Check Graft freshness if tool advertised
      const freshnessTool = toolNames.find((t) => t === "graft_check_freshness" || t === "check_freshness");
      if (freshnessTool) {
        try {
          freshness = await client.callTool(freshnessTool, { path: canonicalRepo });
          if (freshness && (freshness.fresh === false || freshness.synced === false || freshness.status === "stale" || freshness.status === "unindexed")) {
            isIndexed = false;
          }
        } catch {
          // Failure checking freshness degrades indexed confidence
        }
      }

      const projectName = freshness?.project || freshness?.project_name || path.basename(canonicalRepo);
      const generation = freshness?.generation || freshness?.version || null;
      const lastIndexedAt = freshness?.last_indexed_at || freshness?.lastIndexedAt || freshness?.updated_at || null;
      ...
      return {
        ...
        lastIndexedAt,
        capabilities: toolNames,
        warning: isIndexed ? null : "Graft graph is stale or unindexed"
      };
    } catch (err) {
      return {
        provider: this.name,
  ...
  async getArchitecture(options = {}) {
    ...
    const toolName = client.toolSchemas.has("graft_repo_map")
      ? "graft_repo_map"
      : (client.toolSchemas.has("repo_map") ? "repo_map" : (client.toolSchemas.has("describe_repo") ? "describe_repo" : "get_architecture"));

    const raw = await client.callTool(toolName, {
      path: canonicalRepo,
      project: projectName
    });
    ...
    if (!raw || typeof raw !== "object") {
      return {
        ...
        boundaries: [],
        clusters: [],
        evidence: typeof raw === "string" ? [raw] : []
      };
    }
  ...
  async searchCode(query, options = {}) {
    ...
    let toolName = "graft_find_code";
    if (client.toolSchemas.has("graft_find_code")) {
      toolName = "graft_find_code";
    } else if (client.toolSchemas.has("graft_find_all")) {
      toolName = "graft_find_all";
    } else if (client.toolSchemas.has("find_code")) {
      toolName = "find_code";
    } else if (client.toolSchemas.has("find_all")) {
      toolName = "find_all";
    } else if (client.toolSchemas.has("search_symbols")) {
      toolName = "search_symbols";
    } else if (client.toolSchemas.has("search_graph")) {
    ...
    const items = Array.isArray(raw?.nodes)
      ? raw.nodes
      : (Array.isArray(raw?.results) ? raw.results : (Array.isArray(raw?.matches) ? raw.matches : (Array.isArray(raw) ? raw : [])));
    ...
    const matches = items.slice(0, limit).map((m) => {
      ...
      return {
        symbol,
        kind: m.kind || m.type || m.label || "symbol",
        file,
        line,
        qualifiedName: m.qualified_name || m.qualifiedName || (file && symbol ? `${file}:${symbol}` : null),
        score: typeof m.score === "number" ? Number(m.score.toFixed(4)) : (typeof m.rank === "number" ? Number((1 / (1 + m.rank)).toFixed(4)) : 1.0),
        evidence: typeof m.evidence === "string" ? m.evidence.slice(0, 300) : (typeof m.snippet === "string" ? m.snippet.slice(0, 300) : (m.signature ? String(m.signature).slice(0, 300) : null))
      };
    }).filter((m) => m.file || m.symbol);
    ...
    return {
      query,
      matches,
      coverage: raw?.coverage || raw?.freshness || (matches.length > 0 ? "covered" : "unknown")
    };
  }
  ...
  async tracePath(request = {}) {
    ...
    const toolName = client.toolSchemas.has("graft_trace_calls")
      ? "graft_trace_calls"
      : (client.toolSchemas.has("trace_calls") ? "trace_calls" : (client.toolSchemas.has("trace_call_graph") ? "trace_call_graph" : "trace_path"));
    ...
    return {
      ...
      callees,
      paths,
      coverage: raw?.coverage || (callers.length > 0 || callees.length > 0 ? "covered" : "unknown")
    };
  }
  ...
  async detectChanges(request = {}) {
    ...
    const toolName = client.toolSchemas.has("graft_detect_changes")
      ? "graft_detect_changes"
      : (client.toolSchemas.has("detect_changes") ? "detect_changes" : (client.toolSchemas.has("analyze_impact") ? "analyze_impact" : (client.toolSchemas.has("impact_analysis") ? "impact_analysis" : null)));

    let raw = null;
    if (toolName) {
      raw = await client.callTool(toolName, {
        git_diff: diff,
        files: changedFiles,
        path: canonicalRepo,
        project: request.projectName || request.project,
    ...
    const affectedSymbols = Array.isArray(raw?.affected_symbols || raw?.affectedSymbols)
      ? (raw.affected_symbols || raw.affectedSymbols)
      : [];
    const callers = Array.isArray(raw?.callers) ? raw.callers : [];
    const dependents = Array.isArray(raw?.dependents) ? raw.dependents : [];

    return {
      changedFiles: Array.isArray(raw?.changed_files || raw?.changedFiles) ? (raw.changed_files || raw.changedFiles) : changedFiles,
      affectedSymbols,
      callers,
      dependents,
      risk: raw?.risk || "low",
      reasons: Array.isArray(raw?.reasons) ? raw.reasons : ["Graft changeset analysis"],
      coverage: raw?.coverage || "covered"
    };
  }
  ...
  async checkCoverage(request = {}) {
    ...
    const toolName = client.toolSchemas.has("graft_check_freshness")
      ? "graft_check_freshness"
      : (client.toolSchemas.has("check_freshness") ? "check_freshness" : (client.toolSchemas.has("check_index_coverage") ? "check_index_coverage" : null));
    ...
    if (!toolName) {
      return {
        ...
        gaps: [],
        coverageRatio: 0.0,
        warnings: ["Graft coverage/freshness tool is unavailable"]
      };
    }
    ...
    if (raw?.fresh === true || raw?.synced === true || raw?.status === "fresh" || raw?.status === "covered") {
      status = "covered";
      coverageRatio = 1.0;
    } else if (raw?.fresh === false || raw?.status === "stale" || raw?.status === "partial") {
      status = "partial";
      coverageRatio = typeof raw?.coverageRatio === "number" ? raw.coverageRatio : 0.5;
      if (warnings.length === 0) {
        warnings.push("Graft graph is stale or out of sync with current repository files");
      }
    } else if (typeof raw?.coverage_ratio === "number" || typeof raw?.coverageRatio === "number") {
      coverageRatio = raw.coverage_ratio ?? raw.coverageRatio;
      status = coverageRatio === 1 ? "covered" : (coverageRatio > 0 ? "partial" : "unknown");
    }
  ...
  async getSnippet(request = {}) {
    ...
    const symbol = request.symbol || request.symbolName;
    const qualifiedName = request.qualified_name || request.qualifiedName || (
      targetFile && symbol ? `${targetFile}:${symbol}` : targetFile
    );
    if (!qualifiedName && !targetFile) throw new Error("Qualified name or file path is required for getSnippet");

    const client = this._getClient(canonicalRepo);
    await client.connect();

    const toolName = client.toolSchemas.has("graft_file_api")
      ? "graft_file_api"
      : (client.toolSchemas.has("file_api") ? "file_api" : (client.toolSchemas.has("get_code_snippet") ? "get_code_snippet" : (client.toolSchemas.has("graft_get_symbol") ? "graft_get_symbol" : "graft_find_code")));

    const raw = await client.callTool(toolName, {
      file: targetFile || qualifiedName,
      path: canonicalRepo,
      symbol,
      qualified_name: qualifiedName,
      question: qualifiedName
    });

    const content = typeof raw?.content === "string"
      ? raw.content
      : (typeof raw === "string" ? raw : (typeof raw?.api === "string" ? raw.api : (Array.isArray(raw?.signatures) ? raw.signatures.join("\n") : "")));
