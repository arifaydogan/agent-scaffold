...
export class McpCodeIntelligenceProvider extends CodeIntelligenceProvider {
  ...
  async health(repoPath = null) {
    ...
    const rawRepo = repoPath || this.config.cwd || process.cwd();
    let canonicalRepo;
    try {
      canonicalRepo = fs.realpathSync(rawRepo);
    } catch {
      canonicalRepo = path.resolve(rawRepo);
    }
  ...
  async detectChanges(request = {}) {
    ...
    const raw = await client.callTool("detect_changes", {
      project: request.projectName || request.project,
      git_diff: request.diff || request.gitDiff || null,
  ...
  async getSnippet(request = {}) {
    ...
    const raw = await client.callTool("get_code_snippet", {
      ...
      file_path: request.file || request.filePath || request.file_path,
      symbol_name: request.symbol || request.symbolName
    });
...
export function createCodeIntelligenceProvider(settings, runtime = {}) {
  ...
  if (config.type === "graft-mcp") {
    throw new Error("Graft provider is not yet supported in this version of Agent Scaffold");
  }
