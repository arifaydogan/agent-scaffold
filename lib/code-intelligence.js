...
export class GraftCodeIntelligenceProvider extends CodeIntelligenceProvider {
  ...
  async tracePath(request = {}) {
    ...
    if (direction === "both" || direction === "in") {
      try {
        const inRaw = await this._execGraft(canonicalRepo, "callers", [symbol, "--direction", "in", "--depth", String(depth)]);
    ...
    if (direction === "both" || direction === "out") {
      try {
        const outRaw = await this._execGraft(canonicalRepo, "callers", [symbol, "--direction", "out", "--depth", String(depth)]);
  ...
  async detectChanges(request = {}) {
    ...
    for (const file of boundedFiles) {
      try {
        const traceRaw = await this._execGraft(canonicalRepo, "callers", [file, "--direction", "in", "--depth", "2"]);
...
export async function collectReviewIntelligence(settings, issue, changedFiles = [], options = {}) {
  ...
  const repoPath = options.repoPath || options.worktree || settings.repoPath || process.cwd();
  try {
