...
function run(runtime, command, args, cwd) {
...
function git(runtime, repoPath, args) {
  return run(runtime, "git", ["-c", `safe.directory=${repoPath}`, ...args], repoPath);
}
...
export function integrateLeafToEpic(settings, request, options = {}) {
  ...
  const source = git(runtime, repoPath, ["rev-parse", `${sourceBranch}^{commit}`]);
  ...
  const verification = run(runtime, npmCommand, ["run", "check"], repoPath);
