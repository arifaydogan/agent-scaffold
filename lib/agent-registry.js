...
export function validateAgentDefinition(def, { isUpdate = false } = {}) {
  ...
  return {
    ...
    executor: def.executor ? {
      provider: def.executor.provider ? String(def.executor.provider).trim() : null,
      modelProfile: def.executor.modelProfile ? String(def.executor.modelProfile).trim() : null,
      model: def.executor.model ? String(def.executor.model).trim() : null
    } : { provider: null, modelProfile: null, model: null },
...
export const BUILTIN_AGENT_SEEDS = Object.freeze([
  {
    ...
    executor: { provider: null, modelProfile: null, model: null },
  ...
  },
  {
    ...
    role: "architect",
    defaultPersona: "startup-cto",
    skills: ["system-design", "adr-patterns", "tech-debt", "minimal-change"],
    capabilities: ["code-intelligence", "git"],
    executor: { provider: null, modelProfile: "high", model: null },
    reviewer: "correctness-reviewer",
    risk: "normal",
    maxConcurrency: 1,
  ...
  },
  {
    ...
    risk: "normal",
    maxConcurrency: 2,
    allowedPaths: ["frontend/**", "ui/**", "test/ui.test.js"]
  },
  {
    id: "cv-engineer",
  ...
  {
    id: "pm-analyst",
    displayName: "Product Management Analyst",
    status: "enabled",
    role: "specialist",
    defaultPersona: "product-manager",
