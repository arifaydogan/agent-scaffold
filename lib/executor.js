...
export function selectReviewProfile(settings, issue) {
  const labels = issue.labels || [];
  if (!labels.includes("review-claude") && !labels.includes("claude-review")) return null;
  const config = configuredExecutors(settings).antigravity;
  const model = config?.modelProfiles?.["claude-review"];
  if (!config || !model) throw new Error("Claude review requested but antigravity.claude-review is not configured");
  return {
    provider: "antigravity",
    config,
    persona: "qa-engineer",
    taskAgent: "qa-engineer",
    agent: "qa-engineer", // Keep agent for backwards compatibility
    model,
