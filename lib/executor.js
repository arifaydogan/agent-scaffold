 ...
 */
export function selectReviewProfile(settings, issue) {
  const reviewPolicy = settings.data.policy?.review;
  if (!reviewPolicy || !reviewPolicy.provider) return null;
  
  const provider = reviewPolicy.provider;
  const config = configuredExecutors(settings)[provider];
  if (!config) throw new Error(`Review provider '${provider}' is not configured`);
  
  const modelProfile = reviewPolicy.modelProfile || "medium";
  const model = config.modelProfiles?.[modelProfile] || config.defaultModel;
  if (!model) throw new Error(`Unknown model profile '${modelProfile}' for review provider '${provider}'`);
  
  return {
    provider,
    config,
    persona: reviewPolicy.persona || "qa-engineer",
    taskAgent: reviewPolicy.taskAgent || reviewPolicy.persona || "qa-engineer",
    agent: reviewPolicy.taskAgent || reviewPolicy.persona || "qa-engineer",
    model,
    modelProfile,
    effort: reviewPolicy.effort || config.defaultEffort || "medium",
    mode: config.mode || "accept-edits",
    reviewOnly: true
  };
}
