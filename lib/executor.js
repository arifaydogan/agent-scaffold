...
export function parseExecutionOutput(provider, stdout, stderr, returnCode) {
  ...
  if (provider !== "antigravity") {
    ...
    const result = parsed && typeof parsed === "object" && parsed.result !== undefined ? parsed.result : parsed;
    const usage = parsed && typeof parsed === "object" && parsed.usage ? parsed.usage : null;
    const durationSeconds = parsed && typeof parsed === "object" && parsed.duration_seconds ? parsed.duration_seconds : null;
    const schemaValid = validExecutionResult(result);
    const ok = returnCode === 0 && (result ? schemaValid : Boolean(stdout && stdout.trim()));
    let error = null;
    if (!ok) {
      let category = "executor_error";
      if (returnCode !== 0) category = "provider_error";
      else if (!parsed && stdout && stdout.trim()) category = "json_parse_error";
  ...
  if (!ok) {
    ...
    else if (!result && stdout && stdout.trim()) category = "json_parse_error";
    else if (!stdout || !stdout.trim()) category = "empty_output_error";
    else if (!schemaValid) category = "schema_validation_error";
