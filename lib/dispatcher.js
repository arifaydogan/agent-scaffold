...
export async function dispatchOnce(settings, options = {}) {
  const {
    ...
    runIssueImpl = options.runIssue || options.runIssueImpl || runIssue
  } = options;
  ...
  const reconciliation = tick(settings, store, {
    ...
    ...(options.sourceControl ? { sourceControl: options.sourceControl } : {}),
    ...(integrationAdapter ? { integrationAdapter } : {}),
    ...(options.afterIntegrationEvidence ? { afterIntegrationEvidence: options.afterIntegrationEvidence } : {})
  });
  ...
  if (store) {
    for (const issue of issues) {
      ...
      if (iType === "epic" || iType === "epik" || issue.epic || issue.isEpic) {
        try {
    ...
    if (typeof store.listParentExecutions === "function") {
      const parents = store.listParentExecutions(50) || [];
      for (const p of parents) {
        if (p.state === "active") {
          try {
            reconcileParentChildren(settings, store, p.parentKey, { execute, runtime: options.runtime });
          } catch {}
        }
      }
    }
  ...
  const currentPlans = new Map(await Promise.all(issues.map(async (issue) => {
    ...
    const executionPlan = { ...plan, action, attempt };
    executionPlan.planFingerprint = computePlanFingerprint(executionPlan);
    return [issue.key, executionPlan];
  })));
  ...
  const plans = [...retryPlans, ...ordinaryPlans].map((plan) => {
    ...
    const planFingerprint = plan.planFingerprint || computePlanFingerprint(plan);
    const terminalRun = issueRuns.find((r) => {
      if (!["completed", "reviewed-clean", "blocked"].includes(r.state)) return false;
      const role = r.payload?.role || (r.payload?.type === "review" ? "reviewer" : (r.payload?.attempt > 0 ? "rework" : "implementation"));
      const runAction = r.payload?.action || role;
      if (runAction !== (plan.action || "implementation")) return false;
      if ((r.payload?.attempt ?? 0) !== (plan.attempt ?? 0)) return false;
      const runFingerprint = r.payload?.planFingerprint || computePlanFingerprint(r.payload);
      return runFingerprint === planFingerprint;
    });
  ...
  const results = await executeDispatchWaves(waves, async (plan) => {
    ...
    try {
      ...
      if (store && store.database) {
        try {
          const issueKeyToLookup = plan.issue || plan.issueKey || issue.key;
          const row = store.database.prepare("SELECT * FROM epic_tasks WHERE issue_key = ? LIMIT 1").get(issueKeyToLookup);
          if (row && row.epic_key) {
            recordParentTelemetryEvent(store, {
              parentKey: row.epic_key,
              event: "child_dispatched",
              stage: "progress",
              status: "running",
              payload: { issueKey: issueKeyToLookup, childBaseSha: row.child_base_sha, branch: row.branch }
            });
          }
        } catch {}
      }
