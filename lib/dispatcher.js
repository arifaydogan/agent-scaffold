...
export async function dispatchOnce(settings, options = {}) {
  ...
  const currentPlans = new Map(issues.map((issue) => {
    const plan = issuePlan(settings, issue, { store });
    const mode = settings.data?.project?.operatingMode || settings.data?.policy?.operatingMode;
    if (mode === "supervised" && (issue.canonicalState === "ready" || issue.canonicalState === "rework")) {
      const existing = store.getPmDecisions(issue.key);
      if (!existing.some((d) => d.type === "approval_requested" || d.type === "execution_approval")) {
        store.addPmDecision(issue.key, "approval_requested", {
          state: "awaiting-approval",
          summary: issue.summary,
          plan: { issue: issue.key, summary: issue.summary }
        });
      }
    }
