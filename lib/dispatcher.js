...
export async function dispatchOnce(settings, options = {}) {
  ...
  const results = await executeDispatchWaves(waves, async (plan) => {
    ...
    try {
      ...
      if (store && store.database) {
        try {
          const row = store.database.prepare("SELECT * FROM epic_tasks WHERE issue_key = ? LIMIT 1").get(plan.issue);
          if (row && row.epic_key) {
            recordParentTelemetryEvent(store, {
              ...
              stage: "progress",
              status: "running",
              payload: { issueKey: plan.issue, childBaseSha: row.child_base_sha, branch: row.branch }
            });
          }
        } catch {}
      }
