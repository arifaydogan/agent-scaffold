...
function retryPlanMatches(requested, current) {
  return RETRY_PLAN_FIELDS.every(
    (field) => JSON.stringify(stable(requested[field])) === JSON.stringify(stable(current[field]))
  );
}
...
export async function dispatchOnce(settings, options = {}) {
  ...
  // Reconcile durable local state before relying on Jira availability. Recovery,
  // review, and retry requests must continue to advance during a Jira outage.
  ...
  const currentPlans = new Map(issues.map((issue) => [issue.key, issuePlan(settings, issue)]));

  const ordinaryPlans = issues
    .filter((issue) => !retryIssueKeys.has(issue.key))
    .map((issue) => currentPlans.get(issue.key));
  const retryPlans = [];
  for (const request of reconciliation.retries.ready) {
    const current = currentPlans.get(request.issueKey);
    if (!current) {
      blockRetryRequest(store, request.runId, "Retry issue packet was not returned by Jira poll");
      continue;
    }
  ...
  const plans = [...retryPlans, ...ordinaryPlans].map((plan) => {
    ...
    const provider = plan.execution?.provider || "unconfigured";
    ...
    if (
      ...
      reasons.push("Provider " + provider + " capacity is full");
    }
    ...
    return reasons.length
      ? { ...plan, eligible: false, eligibilityReasons: [...new Set(reasons)] }
      : plan;
  });

  const availableWorkers = Math.max(0, maxConcurrency - reconciliation.workers.active);
  const waves = availableWorkers > 0
    ? buildDispatchWaves(plans, {
        maxConcurrency: Math.min(maxConcurrency, availableWorkers),
  ...
  const results = await executeDispatchWaves(waves, async (plan) => {
    ...
    let outcome;
    try {
      outcome = await Promise.resolve(runIssueImpl(settings, issue, true));
    } catch (error) {
