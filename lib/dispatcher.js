...
import { createWorkSourceProvider } from "./work-source.js";
...
export async function dispatchOnce(settings, options = {}) {
  ...
  const issues = await workSource.poll({
    ...
    limit,
    canonicalStates: ["ready", "rework", "review", "human_approval"]
  });
  ...
  const plans = [...retryPlans, ...ordinaryPlans].map((plan) => {
    ...
    let provider = plan.execution?.provider || "unconfigured";
    ...
    return reasons.length
      ? { ...plan, eligible: false, eligibilityReasons: [...new Set(reasons)], dispatchProvider: provider }
      : { ...plan, dispatchProvider: provider };
  });
  ...
  const results = await executeDispatchWaves(waves, async (plan) => {
    ...
    let outcome;
    try {
      if (execute && workSource) {
        await safeWorkSourceMutate(workSource, settings, "transition", issue.key, "in_progress", {}).catch(() => {});
      }
