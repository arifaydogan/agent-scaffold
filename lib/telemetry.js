...
export function buildObservabilitySummary(settings, options = {}) {
  ...
  for (const run of detailedRuns) {
    ...
    let durSeconds = 0;
    if (terminalTelem?.durationMs) {
      durSeconds = Math.round(terminalTelem.durationMs / 1000);
    } else if (startedTelem && terminalTelem) {
      durSeconds = Math.round(Math.max(0, new Date(terminalTelem.timestamp).getTime() - new Date(startedTelem.timestamp).getTime()) / 1000);
    } else if (run.worker_started_at && run.latest_event_at) {
      durSeconds = Math.round(Math.max(0, new Date(run.latest_event_at).getTime() - new Date(run.worker_started_at).getTime()) / 1000);
    }
