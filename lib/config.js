...
import { normalizeProjectProfiles } from "./project-profiles.js";
...
export function loadSettings(configPath) {
  ...
  const projectProfiles = normalizeProjectProfiles(data, base);
  ...
  if (
    data.controlPlane?.hiddenIssueKeys !== undefined &&
    (!Array.isArray(data.controlPlane.hiddenIssueKeys) ||
      data.controlPlane.hiddenIssueKeys.some(value => typeof value !== "string" || !value.trim()))
  ) {
    throw new Error("controlPlane.hiddenIssueKeys must be an array of non-empty issue keys");
  }
  const trustedModelEndpoints = (data.controlPlane?.trustedModelEndpoints || [])
    .map(parseLocalExecutorEndpointOrigin);
  const hiddenIssueKeys = [...new Set((data.controlPlane?.hiddenIssueKeys || [])
  ...
  controlPlane.hiddenIssueKeys = hiddenIssueKeys;
  return {
    source,
