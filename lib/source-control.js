...
export class LocalGitSourceControlProvider extends SourceControlProvider {
  ...
  isAncestor(ancestorSha, descendantSha, options = {}) {
    let ancestor = ancestorSha;
    let descendant = descendantSha;
    let opts = options || {};
    let calledAsObject = false;
    if (typeof ancestorSha === "object" && ancestorSha !== null && !descendantSha) {
      calledAsObject = true;
      opts = ancestorSha;
      ancestor = opts.ancestorSha;
      descendant = opts.descendantSha;
    }
    const cwd = opts.worktree || opts.repoPath;
    const runtime = opts.runtime || { spawnSync };
    const result = runtime.spawnSync(
      "git",
      ["-c", `safe.directory=${cwd}`, "-C", cwd, "merge-base", "--is-ancestor", ancestor, descendant],
      { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }
    );
    const isAnc = result.status === 0;
    if (calledAsObject) {
      return { ok: true, isAncestor: isAnc };
    }
    return isAnc;
  }
  ...
  integrateReviewedRevision(settings, request, options = {}) {
    ...
    const effectiveOptions = {
      ...options,
      runtime: request.runtime || options.runtime || settings.runtime || { spawnSync }
    };
    return integrateLeafToEpic(effectiveSettings, request, effectiveOptions);
  }
}
