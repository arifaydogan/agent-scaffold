...
export class LocalGitSourceControlProvider extends SourceControlProvider {
  ...
  prepareChildWorktree({
    ...
    if (execute) {
      ...
      // Check if child worktree already registered
      const worktreeExists = (runtime.spawnSync || spawnSync)(
        "git",
        ["-c", `safe.directory=${repoPath}`, "-C", repoPath, "worktree", "list", "--porcelain"],
        { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }
      );
      const isRegistered = String(worktreeExists.stdout || "").includes(worktree);
      ...
      if (baseSha) {
        ...
        if (!wtHead.ok || wtHead.sha.toLowerCase() !== baseSha.toLowerCase()) {
          // If worktree was just branched from baseSha, HEAD must match
          if (wtHead.ok && !this.isAncestor(baseSha, wtHead.sha, { repoPath, runtime })) {
