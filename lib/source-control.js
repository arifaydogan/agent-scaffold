...
export class LocalGitSourceControlProvider extends SourceControlProvider {
  ...
  prepareIntegrationWorktree({
    ...
    if (execute) {
      ...
      // Check if worktree directory already exists and can be reused
      const worktreeExists = (runtime.spawnSync || spawnSync)(
        "git",
        ["-c", `safe.directory=${repoPath}`, "-C", repoPath, "worktree", "list", "--porcelain"],
        { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }
      );
      const wtList = String(worktreeExists.stdout || "");
      const isRegistered = wtList.includes(worktree);

      if (isRegistered) {
        // Explicitly verify expected branch, clean state, and correct ancestry
  ...
  prepareChildWorktree({
    ...
    if (execute) {
      ...
      // Check if child worktree already registered or directory exists
      ...
      if (!isRegistered) {
        ...
        isRegistered = String(worktreeExists.stdout || "")
          .split(/\r?\n/)
