...
export function prepareEpicWorktree({ repoPath, root, epicKey, summary, baseRef = "develop", execute = false, runtime = { spawnSync } }) {
  const branch = epicBranchName(epicKey, summary);
  const worktree = path.join(root, branch.replaceAll("/", "-"));
  const base = resolveBaseRef({ repoPath, requestedRef: baseRef, runtime });
  const command = ["git", "-c", `safe.directory=${repoPath}`, "worktree", "add", "-b", branch, worktree, base.ref];
  if (execute) {
    if (!base.resolved) throw new Error(base.error);
  ...
  return { epicKey, branch, worktree, baseRef: base.ref, baseRefResolved: base.resolved, baseRefError: base.error, command };
}

export function prepareLeafWorktree({ repoPath, root, epicKey, epicBranch, issueKey, summary, execute = false, runtime = { spawnSync } }) {
  const branch = leafBranchName(issueKey, summary);
  const worktree = path.join(root, branch.replaceAll("/", "-"));
  const base = resolveBaseRef({ repoPath, requestedRef: epicBranch, epicKey, runtime });
  const command = ["git", "-c", `safe.directory=${repoPath}`, "worktree", "add", "-b", branch, worktree, base.ref];
  if (execute) {
    if (!base.resolved) throw new Error(base.error);
    const result = runtime.spawnSync(command[0], command.slice(1), { cwd: repoPath, stdio: "inherit" });
  ...
  return { epicKey, issueKey, branch, worktree, baseRef: base.ref, baseRefResolved: base.resolved, baseRefError: base.error, command };
}
