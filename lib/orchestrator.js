...
import { extractDeclaredAllowedPaths, routeIssue } from "./routing.js";
import { normalizeUsage, redactTelemetryPayload } from "./telemetry.js";
import { BUILTIN_AGENT_SEEDS } from "./agent-registry.js";
...
export class BuiltinOrchestratorProvider extends OrchestratorProvider {
  ...
  plan(issue, options = {}) {
    ...
    let paths;
    if (policyScopes === undefined || policyScopes === null) {
      const seedDef = BUILTIN_AGENT_SEEDS.find((s) => s.id === taskAgent);
      paths = seedDef?.allowedPaths ? [...seedDef.allowedPaths] : ["lib/**", "src/**"];
    } else {
      paths = policyScopes[taskAgent] || [];
    }
    const declaredPaths = extractDeclaredAllowedPaths(issue.description);
    if (declaredPaths.length > 0) paths = declaredPaths;
...
export class CliOrchestratorProvider extends OrchestratorProvider {
  ...
  buildPrompt(issue, options = {}) {
    ...
    const codeIntel = options.codeIntelligence;
    let codeIntelBlock = "";
    if (codeIntel && codeIntel.status !== "disabled" && codeIntel.status !== "unavailable") {
      const parts = [
        "CODE INTELLIGENCE:",
        `- Provider: ${codeIntel.provider || "codebase-memory"}${codeIntel.generation ? ` (gen: ${codeIntel.generation})` : ""}`,
        `- Coverage: ${codeIntel.coverage?.status || "covered"}`
      ];
      if (codeIntel.architecture?.relevantPackages?.length) {
        parts.push(`- Relevant packages: ${codeIntel.architecture.relevantPackages.join(", ")}`);
      }
      if (codeIntel.search?.files?.length) {
        parts.push(`- Relevant files: ${codeIntel.search.files.join(", ")}`);
      }
      if (codeIntel.search?.symbols?.length) {
        parts.push(`- Relevant symbols: ${codeIntel.search.symbols.join(", ")}`);
      }
      if (codeIntel.impact?.blastRadius) {
        parts.push(`- Likely blast radius: ${codeIntel.impact.blastRadius}`);
      }
      codeIntelBlock = parts.join("\n");
    }
  ...
  plan(issue, options = {}) {
    ...
    const store = options.store || options.settings?._store || this.config.store || null;
    const issueKey = issue.key || issue.id || "TASK-1";
    let planningRunId = options.runId || options.planningRunId || null;
    let createdLocalRun = false;

    if (store) {
      if (!planningRunId) {
        planningRunId = store.createRun(issueKey, {
          role: "orchestrator",
          action: "planning",
      ...
      if (typeof store.recordTelemetryEvent === "function") {
        ...
        store.recordTelemetryEvent({
          eventId: `telem-${planningRunId}-2-started`,
          runId: planningRunId,
          issueKey,
          role: "orchestrator",
          action: "planning",
    ...
    try {
      ...
      if (store && planningRunId && typeof store.recordTelemetryEvent === "function") {
        store.recordTelemetryEvent({
          eventId: `telem-${planningRunId}-term-err`,
          runId: planningRunId,
          issueKey,
          role: "orchestrator",
          action: "planning",
          ...
          status: "failed",
          sequence: 999,
          error: { category: "provider_process_error", safeMessage: err.message }
        });
        if (createdLocalRun) store.transition(planningRunId, "failed", { reason: err.message });
      }
      if (err.code === "ENOENT") {
    ...
    if (result.error) {
      if (store && planningRunId && typeof store.recordTelemetryEvent === "function") {
        store.recordTelemetryEvent({
          ...
          status: "failed",
          sequence: 999,
          error: { category: "provider_process_error", safeMessage: result.error.message }
        });
        if (createdLocalRun) store.transition(planningRunId, "failed", { reason: result.error.message });
      }
    ...
    if (result.status !== 0) {
      if (store && planningRunId && typeof store.recordTelemetryEvent === "function") {
        store.recordTelemetryEvent({
          ...
          status: "failed",
          sequence: 999,
          error: { category: "provider_process_error", safeMessage: `Exit code ${result.status}` }
        });
        if (createdLocalRun) store.transition(planningRunId, "failed", { reason: `Exit code ${result.status}` });
      }
    ...
    if (!stdout) {
      if (store && planningRunId && typeof store.recordTelemetryEvent === "function") {
        store.recordTelemetryEvent({
          ...
          status: "failed",
          sequence: 999,
          error: { category: "telemetry_parse_error", safeMessage: "Orchestrator returned empty stdout" }
        });
        if (createdLocalRun) store.transition(planningRunId, "failed", { reason: "Orchestrator returned empty stdout" });
      }
    ...
    try {
      ...
      if (store && planningRunId && typeof store.recordTelemetryEvent === "function") {
        store.recordTelemetryEvent({
          ...
          status: "failed",
          sequence: 999,
          error: { category: "telemetry_parse_error", safeMessage: parseErr.message }
        });
        if (createdLocalRun) store.transition(planningRunId, "failed", { reason: parseErr.message });
      }
      throw parseErr;
    }
    ...
    let validated;
    try {
      validated = validateOrchestratorPlan(parsed, { issue });
    } catch (valErr) {
      if (store && planningRunId && typeof store.recordTelemetryEvent === "function") {
        const normUsage = normalizeUsage(parsed?.usage || null);
        store.recordTelemetryEvent({
          eventId: `telem-${planningRunId}-term-err`,
          runId: planningRunId,
          issueKey,
          ...
          sequence: 999,
          usage: normUsage,
          error: { category: "policy_block", safeMessage: valErr.message }
        });
        if (createdLocalRun) store.transition(planningRunId, "failed", { reason: valErr.message });
      }
      throw valErr;
    }
    ...
    if (store && planningRunId && typeof store.recordTelemetryEvent === "function") {
      const normUsage = normalizeUsage(parsed?.usage || null);
      store.recordTelemetryEvent({
        eventId: `telem-${planningRunId}-term-ok`,
        runId: planningRunId,
        issueKey,
        role: "orchestrator",
        action: "planning",
        attempt: 0,
        persona: validated.persona || "architect",
        taskAgent: validated.taskAgent || "orchestrator",
        provider: this.name,
        stage: "terminal",
        status: "completed",
        sequence: 999,
      ...
      if (createdLocalRun) store.transition(planningRunId, "completed", { returnCode: 0, plan: validated });
    }

    if (planningRunId) {
      validated.metadata = { ...(validated.metadata || {}), planningRunId };
    }

    validated.codeIntelligence = options.codeIntelligence || null;
...
export class AntigravityOrchestratorProvider extends CliOrchestratorProvider {
  ...
  parseOutput(stdout) {
    const lines = stdout.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
    let terminalSuccessResponse = null;
    ...
    for (const line of lines) {
      try {
        ...
        // ONLY terminal SUCCESS (or ok === true when status is not PROGRESS/QUEUED/PENDING) provides the plan
        if (event.status === "SUCCESS" || (event.ok === true && !["PROGRESS", "QUEUED", "PENDING"].includes(event.status))) {
          terminalSuccessResponse = event.response || event.result || event;
        }
      } catch (err) {
        if (err instanceof OrchestratorProcessError) throw err;
      }
    ...
    if (terminalSuccessResponse) {
      if (typeof terminalSuccessResponse === "object") return terminalSuccessResponse;
      return extractJsonObject(terminalSuccessResponse, this.name);
    }

    // Direct JSON object fallback if single non-stream output with SUCCESS or explicit plan
    try {
      const top = JSON.parse(stdout.trim());
      if (top && typeof top === "object" && top.status === "SUCCESS" && top.response) {
        return typeof top.response === "object" ? top.response : extractJsonObject(top.response, this.name);
      }
      if (top && typeof top === "object" && top.persona && top.taskAgent && top.issue) {
        return top;
      }
    } catch {}

    throw new OrchestratorParseError(`Antigravity stream did not reach terminal SUCCESS event`, {
      provider: this.name,
      rawOutput: stdout
    });
  }
