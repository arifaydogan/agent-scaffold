...
export class CodexOrchestratorProvider extends CliOrchestratorProvider {
  ...
  parseOutput(stdout) {
    ...
    // Scan lines as JSONL events
    for (const line of lines) {
      try {
        const event = JSON.parse(line);
        // Codex event protocols: item.completed (message), agent_message, message.completed, response
        if (event.type === "item.completed" && event.item?.type === "message" && event.item?.role === "assistant") {
        ...
        } else if (event.type === "agent_message" || event.type === "message") {
          finalMessage = event.message || event.text || event.content;
        ...
        // Non-JSON line in stdout, ignore event parsing and try text extraction
      }
    }
