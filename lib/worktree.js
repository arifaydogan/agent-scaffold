...
export function prepareWorktree({
  ...
  if (execute) {
    ...
    // Check if worktree directory or registration already exists
    const dirExists = fs.existsSync(worktree);
    let isRegistered = dirExists;
    if (!isRegistered) {
      ...
      const normalizedWorktree = path.resolve(worktree).toLowerCase().replaceAll("\\", "/");
      isRegistered = String(wtListRes.stdout || "")
        .split(/\r?\n/)
        .some((line) => line.startsWith("worktree ") && path.resolve(line.slice(9).trim()).toLowerCase().replaceAll("\\", "/") === normalizedWorktree);
    }
