...
export function createConfigSnapshot(settings, issue, plan = {}, options = {}) {
  ...
  return {
    ...
    autonomy,
    agentId: taskAgent,
    agentVersion: plan?.agentVersion || agentRecord?.version || 1,
...
export function issuePlan(settings, issue, options = {}) {
  ...
  const rawPlan = {
    ...
    agentId: taskAgent,
    agentVersion: snapshot?.agentVersion || agentRecord?.version || 1,
  ...
  const configSnapshot = snapshot || createConfigSnapshot(settings, issue, rawPlan, { store });
