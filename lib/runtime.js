...
import { evaluateIssue, resolveOperatingMode, resolveAutonomyPolicy, computePlanFingerprint } from "./policy.js";
import {
...
import { captureOperatorRequest, getAnsweredOperatorContext } from "./operator-inbox.js";
...
export function createConfigSnapshot(settings, issue, plan = {}, options = {}) {
  ...
  return {
    ...
    executionCandidates: Array.isArray(plan?.executionCandidates)
      ? JSON.parse(JSON.stringify(plan.executionCandidates))
      : [],
    adaptiveRouting: settings.data.executor?.adaptiveRouting
      ? JSON.parse(JSON.stringify(settings.data.executor.adaptiveRouting))
      : null,
    recommendedExecutor,
    recommendedModel,
    recommendedProfile,
    ...
    parentKey: plan?.parentKey || plan?.epicKey || null,
    graphFingerprint: plan?.graphFingerprint || null,
    childBaseSha: plan?.childBaseSha || null,
    ...
    reviewIntelligence: plan?.reviewIntelligence ? JSON.parse(JSON.stringify(plan.reviewIntelligence)) : (options?.reviewIntelligence ? JSON.parse(JSON.stringify(options.reviewIntelligence)) : null),
    originatingCodeIntelligence: plan?.originatingCodeIntelligence ? JSON.parse(JSON.stringify(plan.originatingCodeIntelligence)) : (options?.originatingCodeIntelligence ? JSON.parse(JSON.stringify(options.originatingCodeIntelligence)) : null),
    reworkCodeIntelligence: plan?.reworkCodeIntelligence ? JSON.parse(JSON.stringify(plan.reworkCodeIntelligence)) : (options?.reworkCodeIntelligence ? JSON.parse(JSON.stringify(options.reworkCodeIntelligence)) : null),
...
export function issuePlan(settings, issue, options = {}) {
  ...
  let pinnedEpicTask = null;
  try {
    if (store?.database) {
      const row = store.database
        .prepare("SELECT * FROM epic_tasks WHERE issue_key = ? LIMIT 1")
        .get(issue.key);
      if (row) {
        pinnedEpicTask = {
    ...
    } else if (typeof store?.listEpicTasks === "function") {
      pinnedEpicTask = (store.listEpicTasks(null) || []).find((task) => task.issueKey === issue.key) || null;
    }
  } catch {}
  const issueEpicKey = issue.epicKey || issue.parentKey || issue.fields?.parent?.key || null;
  const epicKey = issueEpicKey || snapshot?.parentKey || snapshot?.epicKey || pinnedEpicTask?.epicKey || null;
  const parentExecution = epicKey && typeof store?.getParentExecution === "function"
    ? store.getParentExecution(epicKey)
    : null;
  let agentRecord = null;
  ...
  const executionCandidates = execution
    ? (Array.isArray(snapshot?.executionCandidates) && snapshot.executionCandidates.length > 0
        ? snapshot.executionCandidates
        : selectExecutionCandidates(settings, issue, {
            ...route,
            risk,
            taskAgent,
            agentExecutor: agentRecord?.definition?.executor || null
  ...
  const configuredBaseBranch = settings?.data?.project?.baseBranch
    || settings?.data?.project?.baseRef
    || settings?.data?.git?.baseRef
    || "HEAD";
  const epicBranch = issue.epicBranch
    || issue.baseBranch
    || issue.baseRef
    || parentExecution?.integrationBranch
    || (epicKey ? "epic/" + epicKey.toLowerCase() : configuredBaseBranch);
  ...
  const rawPlan = {
    ...
    epicKey,
    parentKey: epicKey,
    graphFingerprint: snapshot?.graphFingerprint || parentExecution?.graphFingerprint || null,
    childBaseSha: snapshot?.childBaseSha || pinnedEpicTask?.childBaseSha || null,
    labels: issue.labels || [],
    ...route,
    ...
    reviewIntelligence: options.reviewIntelligence || snapshot?.reviewIntelligence || route.reviewIntelligence || null,
    originatingCodeIntelligence: options.originatingCodeIntelligence || snapshot?.originatingCodeIntelligence || route.originatingCodeIntelligence || null,
    reworkCodeIntelligence: options.reworkCodeIntelligence || snapshot?.reworkCodeIntelligence || route.reworkCodeIntelligence || null,
    requestedAllowedPaths: Array.isArray(route.allowedPaths) ? [...route.allowedPaths] : [],
    allowedPaths,
    execution,
    executionCandidates,
    adaptiveRouting: snapshot?.adaptiveRouting || settings.data.executor?.adaptiveRouting || null,
  ...
  const configSnapshot = snapshot
    ? {
        ...snapshot,
        originatingCodeIntelligence: rawPlan.originatingCodeIntelligence || snapshot.originatingCodeIntelligence || snapshot.codeIntelligence || null,
        reworkCodeIntelligence: rawPlan.reworkCodeIntelligence || snapshot.reworkCodeIntelligence || null
      }
    : createConfigSnapshot(settings, issue, rawPlan, {
        store,
        codeIntelligence: rawPlan.codeIntelligence,
        reviewIntelligence: rawPlan.reviewIntelligence,
        originatingCodeIntelligence: rawPlan.originatingCodeIntelligence,
        reworkCodeIntelligence: rawPlan.reworkCodeIntelligence
      });
...
export async function issuePlanWithIntelligence(settings, issue, options = {}) {
  let codeIntelligence = options.codeIntelligence || null;
  let reviewIntelligence = options.reviewIntelligence || null;

  const originatingSnapshot = options.originatingRun?.payload?.configSnapshot || options.originatingRun?.configSnapshot || null;

  if (options.action === "review" || issue.canonicalState === "review") {
    if (!reviewIntelligence) {
      let changedFiles = options.changedFiles || [];
      if (changedFiles.length === 0 && options.originatingRun?.latest_payload?.implementationSha) {
        const sha = options.originatingRun.latest_payload.implementationSha;
        const repoPath = settings.repoPath || process.cwd();
        try {
          const syncSpawn = options.runtime?.spawnSync || spawnSync;
          const diffRes = syncSpawn("git", ["diff-tree", "--no-commit-id", "--name-only", "-r", sha], { cwd: repoPath, encoding: "utf8" });
          if (diffRes.status === 0 && diffRes.stdout) {
            changedFiles = diffRes.stdout.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
          }
        } catch {}
      }
      try {
        reviewIntelligence = await collectReviewIntelligence(settings, issue, changedFiles, {
          runtime: options.runtime,
          store: options.store || getStore(settings)
        });
      } catch (err) {
        reviewIntelligence = {
          provider: "none",
  ...
  } else if (options.action === "rework" || issue.canonicalState === "rework") {
    const origIntel = options.originatingCodeIntelligence || originatingSnapshot?.originatingCodeIntelligence || originatingSnapshot?.codeIntelligence || options.originatingRun?.payload?.codeIntelligence || null;
    codeIntelligence = origIntel;

    let reworkIntelligence = options.reworkCodeIntelligence || null;
    if (!reworkIntelligence) {
      let changedFiles = options.changedFiles || [];
      let reviewOutcome = options.originatingRun?.latest_payload?.reviewOutcome || options.originatingRun?.payload?.reviewOutcome;
      if (!reviewOutcome && Array.isArray(options.originatingRun?.events)) {
        const ev = [...options.originatingRun.events].reverse().find((e) => e.payload?.reviewOutcome);
        if (ev) reviewOutcome = ev.payload.reviewOutcome;
      }
      if (!reviewOutcome && options.originatingRun?.id) {
        const store = options.store || getStore(settings);
        if (store && typeof store.getRun === "function") {
          try {
            const r = store.getRun(options.originatingRun.id);
            const ev = [...(r.events || [])].reverse().find((e) => e.payload?.reviewOutcome);
            if (ev) reviewOutcome = ev.payload.reviewOutcome;
          } catch {}
        }
      }
      if (Array.isArray(reviewOutcome?.evidence) && changedFiles.length === 0) {
        changedFiles = reviewOutcome.evidence.map((e) => (typeof e === "string" ? e : e.file)).filter(Boolean);
      }
      if (changedFiles.length === 0 && options.originatingRun?.latest_payload?.implementationSha) {
        const sha = options.originatingRun.latest_payload.implementationSha;
        const repoPath = settings.repoPath || process.cwd();
      ...
      try {
        reworkIntelligence = await collectReviewIntelligence(settings, issue, changedFiles, {
          runtime: options.runtime,
          store: options.store || getStore(settings)
        });
      } catch (err) {
    ...
    return issuePlan(settings, issue, {
      ...options,
      codeIntelligence: origIntel,
      originatingCodeIntelligence: origIntel,
      reworkCodeIntelligence: reworkIntelligence
    });
  } else {
    // implementation planning
    if (!codeIntelligence) {
      if (originatingSnapshot?.codeIntelligence) {
        codeIntelligence = originatingSnapshot.codeIntelligence;
      } else {
        try {
          codeIntelligence = await collectCodeIntelligenceContext(settings, issue, {
            runtime: options.runtime,
            store: options.store || getStore(settings)
          });
        } catch (err) {
...
export function spawnProviderAsync(params, runtime = { spawn }) {
  ...
  return new Promise((resolve) => {
    ...
    child.once("close", (code) => {
      ...
      const accepted = telemetry.ok && (telemetry.result?.status === "completed" || telemetry.result?.verdict === "clean") && scope.allowed;
      const failure = accepted ? null : classifyExecutionFailure({ telemetry, returnCode, scope, stdout, stderr: stderrBuffer });
      const failoverEligible = Boolean(failure?.autoFailover && scope.allowed && scope.changedFiles.length === 0);
      const operatorRequest = scope.allowed && !failoverEligible
        ? captureOperatorRequest(store, {
            issueKey,
            runId,
            result: telemetry.result,
            planFingerprint: plan.planFingerprint,
            taskAgent: plan.taskAgent
          })
        : null;
      const terminalState = operatorRequest
      ...
      store.transition(
        ...
        terminalState,
        {
          ...
          durationSeconds: (telemetry.durationSeconds !== null && telemetry.durationSeconds !== undefined && Number.isFinite(Number(telemetry.durationSeconds))) ? Number(telemetry.durationSeconds) : null,
          ...
          humanActionRequired: Boolean(operatorRequest),
          operatorRequest: operatorRequest ? { requestId: operatorRequest.requestId, question: operatorRequest.question } : null,
          reason: accepted ? null : failure?.reason || "Agent execution failed",
          error: accepted ? null : failure?.safeMessage || null,
          failure,
          failoverEligible,
          scope,
      ...
      if (typeof store.recordTelemetryEvent === "function") {
        store.recordTelemetryEvent({
          ...
          model: profile?.model,
          stage: "terminal",
          status: operatorRequest ? "blocked" : accepted ? "completed" : "failed",
          sequence: 999,
          durationMs: (telemetry.durationSeconds !== null && telemetry.durationSeconds !== undefined && Number.isFinite(Number(telemetry.durationSeconds))) ? Math.round(Number(telemetry.durationSeconds) * 1000) : null,
          usage: normUsage,
          error: operatorRequest || accepted ? null : {
            category: failure?.category || (scope.allowed ? "provider_process_error" : "scope_violation"),
            safeMessage: failure?.safeMessage || `Process exited with code ${returnCode}`
          }
        });
      }

      if (operatorRequest || !accepted) store.releaseLock(issueKey, runId);
      resolve({
        exitCode: operatorRequest ? 2 : accepted ? 0 : scope.allowed ? returnCode || 4 : 5,
        telemetry,
        scope,
        operatorRequest,
        failure,
...
export function runIssue(settings, issue, execute, runtime = { spawnSync, spawn }, options = {}) {
  const state = issue.canonicalState;
  if (state === "ready" || state === "in_progress" || state === "in-progress") {
    const plan = options.plan || issuePlan(settings, issue, { ...options, runtime });
    return runImplementationWithFailover(settings, issue, execute, runtime, { ...options, plan });
  }
  switch (state) {
    case "review":
      return handleReview(settings, issue, execute, runtime, options);
    case "rework":
      return handleRework(settings, issue, execute, runtime, options);
    case "human_approval":
      return handleHumanApprovalObservation(settings, issue, execute, runtime, options);
    default:
      return { exitCode: 0, output: { runId: null, mode: "skipped" } };
  }
}

export function runIssueWithPlan(settings, issue, plan, execute, runtime = { spawnSync, spawn }, options = {}) {
  return runIssue(settings, issue, execute, runtime, { ...options, plan });
}

function runImplementationWithFailover(settings, issue, execute, runtime, options) {
  const plan = options.plan;
  const adaptive = plan?.adaptiveRouting || settings.data.executor?.adaptiveRouting || {};
  const candidates = Array.isArray(plan?.executionCandidates) && plan.executionCandidates.length > 0
    ? plan.executionCandidates
    : (plan?.execution ? [plan.execution] : []);
  const maxAttempts = Math.max(1, Math.min(candidates.length, Number(adaptive.maxAttempts || candidates.length)));
  const attempted = new Set();
  const history = [];

  const keyFor = candidate => `${candidate?.provider || ""}\u0000${candidate?.model || ""}`;
  const decorate = result => history.length > 0
    ? { ...result, output: { ...(result.output || {}), failoverHistory: history } }
    : result;

  const chooseNext = (current, failure) => candidates.find(candidate => {
    const key = keyFor(candidate);
    if (attempted.has(key)) return false;
    if (failure?.providerWide && candidate.provider === current?.provider) return false;
    return true;
  }) || null;

  const settle = (value, currentPlan) => {
    if (value && typeof value.then === "function") {
      return value.then(result => settle(result, currentPlan));
    }
    const result = value;
    const current = currentPlan.execution || candidates[0] || {};
    attempted.add(keyFor(current));
    if (!execute || result?.exitCode === 0) return decorate(result);

    const failure = result?.output?.failure || null;
    const canSwitch = adaptive.enabled === true
      && failure?.autoFailover === true
      && result?.output?.scope?.allowed === true
      && (result.output.scope.changedFiles || []).length === 0
      && attempted.size < maxAttempts;
    const next = canSwitch ? chooseNext(current, failure) : null;
    if (!next) return decorate(result);

    history.push({
      runId: result.output?.runId || null,
      provider: current.provider || result.output?.provider || null,
      model: current.model || result.output?.model || null,
      category: failure.category,
      reason: failure.reason,
      nextProvider: next.provider,
      nextModel: next.model || null
    });
    const nextPlan = {
      ...currentPlan,
      execution: next,
      failoverContext: {
        fromRunId: result.output?.runId || null,
        category: failure.category,
        fromProvider: current.provider || null,
        fromModel: current.model || null
      }
    };
    return settle(handleImplementation(settings, issue, execute, runtime, { ...options, plan: nextPlan }), nextPlan);
  };

  return settle(handleImplementation(settings, issue, execute, runtime, options), plan);
}

export function handleHumanApprovalObservation(settings, issue, execute, runtime, options = {}) {
  return { exitCode: 0, output: { runId: null, mode: "human_approval_observation", issue: issue.key } };
}

export function handleRework(settings, issue, execute, runtime, options = {}) {
  ...
  const originatingCodeIntel = originatingSnapshot?.codeIntelligence || retryableRun.payload?.codeIntelligence || null;
  ...
  return handleImplementation(settings, issue, execute, runtime, { 
    ...
    codeIntelligence: originatingCodeIntel,
    originatingCodeIntelligence: originatingCodeIntel,
    reworkCodeIntelligence: options.reworkCodeIntelligence || options.plan?.reworkCodeIntelligence || null
  });
}

export function handleReview(settings, issue, execute, runtime = { spawnSync, spawn }, options = {}) {
  const store = getStore(settings);
  const runs = store.listRunsDetailed(100);
  ...
  const implementationSha = queuedRun.latest_payload?.implementationSha || queuedRun.latest_payload?.commit || queuedRun.payload?.commit;
  ...
  const basePlan = options.plan || issuePlan(settings, issue, { store, action: "review", originatingRun: queuedRun, runtime });
  const plan = options.plan ? { ...options.plan, ...options } : { ...basePlan, ...options };
  const reviewClaimPayload = { ...plan, type: "review", implementationSha, configSnapshot: originatingSnapshot || plan.configSnapshot };

  if (!plan.eligible) {
    const runId = store.createRun(issue.key, reviewClaimPayload);
  ...
  let runId;
  if (typeof store.createRunAndClaimIssue === "function") {
    const claimRes = store.createRunAndClaimIssue(issue.key, reviewClaimPayload);
    if (!claimRes.claimed) {
      return { exitCode: 3, output: { runId: null, error: "issue already locked" } };
    }
    runId = claimRes.runId;
  } else {
    runId = store.createRun(issue.key, reviewClaimPayload);
    store.transition(runId, "eligible", plan);
  ...
  try {
    prepared = prepareWorktree({
      ...
      epicBranch: issue.epicBranch || issue.baseBranch || issue.baseRef || (issue.epicKey ? "epic/" + issue.epicKey.toLowerCase() : configuredBaseBranch),
  ...
  const handleReviewCompletion = ({ exitCode, telemetry, scope }) => {
    try {
      ...
      try {
        const validatedEvidence = (verdict === "clean" && (!Array.isArray(evidence) || evidence.length === 0))
          ? [{ id: "clean-verdict", problem: "Review verdict is clean; no issues detected", severity: "suggestion", category: "correctness" }]
          : evidence;
  ...
  if (typeof store.recordTelemetryEvent === "function") {
    store.recordTelemetryEvent({
      ...
      sequence: 999,
      durationMs: (telemetry.durationSeconds !== null && telemetry.durationSeconds !== undefined) ? Math.round(telemetry.durationSeconds * 1000) : null,
      usage: normUsage,
...
export function handleImplementation(settings, issue, execute, runtime = { spawnSync, spawn }, planOverrides = {}) {
  ...
  const action = planOverrides.role === "rework" || issue?.canonicalState === "rework"
    ? "rework"
    : (planOverrides.action || planOverrides.plan?.action || "implementation");
  const attempt = planOverrides.attempt ?? planOverrides.plan?.attempt ?? 0;
  const originatingRun = planOverrides.originatingRun || null;

  const basePlan = planOverrides.plan || issuePlan(settings, issue, {
    store,
    ...
    originatingRun,
    runtime,
    codeIntelligence: planOverrides.codeIntelligence,
    reviewIntelligence: planOverrides.reviewIntelligence,
    originatingCodeIntelligence: planOverrides.originatingCodeIntelligence,
    reworkCodeIntelligence: planOverrides.reworkCodeIntelligence
  });
  const configSnapshot = {
    ...(basePlan.configSnapshot || {}),
    ...(planOverrides.configSnapshot || {}),
    originatingCodeIntelligence: planOverrides.originatingCodeIntelligence || planOverrides.configSnapshot?.originatingCodeIntelligence || basePlan.configSnapshot?.originatingCodeIntelligence || basePlan.configSnapshot?.codeIntelligence || null,
    reworkCodeIntelligence: planOverrides.reworkCodeIntelligence || planOverrides.configSnapshot?.reworkCodeIntelligence || basePlan.configSnapshot?.reworkCodeIntelligence || null
  };
  const plan = planOverrides.plan ? { ...planOverrides.plan, ...planOverrides, configSnapshot } : { ...basePlan, ...planOverrides, configSnapshot };
  let operatorContext = null;
  if (planOverrides.operatorRequestId) {
    try {
      operatorContext = getAnsweredOperatorContext(store, planOverrides.operatorRequestId);
      if (operatorContext.issueKey !== issue.key) throw new Error("Operator response belongs to another work item");
    } catch (error) {
      return { exitCode: 2, output: { runId: null, error: error.message } };
    }
  }
  ...
  if (action === "implementation" && execute && !planOverrides.force) {
    const currentAttempt = attempt;
    const existingRuns = typeof store.listRunsDetailed === "function"
      ? store.listRunsDetailed(100)
      : (typeof store.listRuns === "function" ? store.listRuns() : []);
    const completedImplementation = existingRuns.find(r => {
      if (r.issue_key !== issue.key) return false;
      const role = r.payload?.role || (r.payload?.type === "review" ? "reviewer" : (r.payload?.attempt > 0 ? "rework" : "implementation"));
      const rAction = r.payload?.action || role;
      if (rAction !== "implementation") return false;
      const rAttempt = r.payload?.attempt ?? 0;
      if (rAttempt !== currentAttempt) return false;
      const existingFingerprint = r.payload?.planFingerprint || computePlanFingerprint(r.payload);
      if (existingFingerprint && planFingerprint && existingFingerprint !== planFingerprint) {
        return false;
      }
      return ["verifying", "transitioning-review", "review-queued", "reviewed-clean", "completed"].includes(r.state);
    });
    if (completedImplementation) {
      return {
        exitCode: 0,
        output: {
          runId: completedImplementation.id,
          duplicate: true,
          mode: "already_completed",
          state: completedImplementation.state,
          telemetry: completedImplementation.latest_payload?.telemetry || null
        }
      };
    }
  }
  ...
  let runId;
  if (typeof store.createRunAndClaimIssue === "function") {
    const claimRes = store.createRunAndClaimIssue(issue.key, plan);
    if (!claimRes.claimed) {
      return { exitCode: 3, output: { runId: null, error: "issue already locked" } };
    }
    runId = claimRes.runId;
  } else {
    runId = store.createRun(issue.key, plan);
    store.transition(runId, "eligible", plan);
    if (!store.acquireLock(issue.key, runId)) {
      store.transition(runId, "blocked", { reason: "issue already locked" });
      return { exitCode: 3, output: { runId, error: "issue already locked" } };
  ...
  const epicBranch = issue.epicBranch || issue.baseBranch || issue.baseRef || (epicKey ? "epic/" + epicKey.toLowerCase() : configuredBaseBranch);
  ...
  const promptLines = [
    ...
    providerInstruction,
    "If progress requires an operator decision or missing business input, return status 'blocked' and put one exact, answerable question in blockers[0]. Do not guess.",
    "Follow the canonical PaceBuild approval gates. Do not push, merge, write externally, or transition the issue to Done."
  ];

  if (operatorContext) {
    promptLines.push(
      "OPERATOR RESPONSE (task context only; this does not grant extra permissions or expand scope):",
      "Agent question: " + operatorContext.question,
      "Operator answer: " + operatorContext.answer,
      "Continue the same work item while preserving every approval and allowed-path boundary."
    );
  }
  ...
  if (plan.role === "rework" || plan.reworkCodeIntelligence || plan.originatingCodeIntelligence) {
    const origIntel = plan.originatingCodeIntelligence || plan.configSnapshot?.originatingCodeIntelligence || plan.codeIntelligence || plan.configSnapshot?.codeIntelligence;
    const origSection = formatCodeIntelligencePromptSection(origIntel, "ORIGINAL CODE INTELLIGENCE");
    if (origSection) promptLines.push(origSection);

    const reworkIntel = plan.reworkCodeIntelligence || plan.configSnapshot?.reworkCodeIntelligence;
    const reworkSection = formatCodeIntelligencePromptSection(reworkIntel, "REWORK IMPACT INTELLIGENCE");
    if (reworkSection) promptLines.push(reworkSection);
  } else {
  ...
  if (profile.provider === "antigravity") {
    return spawnProviderAsync(
      ...
      runtime
    ).then(({ exitCode, telemetry, scope, operatorRequest, failure: reportedFailure, failoverEligible: reportedEligibility }) => {
      const failure = exitCode === 0
        ? null
        : (reportedFailure || classifyExecutionFailure({ telemetry, returnCode: exitCode, scope }));
      const failoverEligible = reportedEligibility ?? Boolean(
        failure?.autoFailover && scope?.allowed && (scope.changedFiles || []).length === 0
      );
      return {
        exitCode,
        output: {
  ...
  const accepted = telemetry.ok && (telemetry.result?.status === "completed" || telemetry.result?.verdict === "clean") && scope.allowed;
  const failure = accepted ? null : classifyExecutionFailure({ telemetry, returnCode, scope, stdout, stderr });
  const failoverEligible = Boolean(failure?.autoFailover && scope.allowed && scope.changedFiles.length === 0);
  const operatorRequest = scope.allowed && !failoverEligible
    ? captureOperatorRequest(store, {
        issueKey: issue.key,
