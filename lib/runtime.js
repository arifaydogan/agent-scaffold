...
export function issuePlan(settings, issue, options = {}) {
  ...
  const policyPaths = settings.data?.policy?.pathScopes?.[route.persona] || [];
  const allowedPaths = snapshot?.allowedPaths !== undefined
    ? snapshot.allowedPaths
    : (Array.isArray(route.allowedPaths)
        ? (policyPaths.length ? intersectPathScopes(route.allowedPaths, policyPaths) : route.allowedPaths)
        : policyPaths);
