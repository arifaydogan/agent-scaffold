  ...
  return new Promise((resolve) => {
    ...
    child.stdout.on("data", (chunk) => {
      ...
      for (const line of lines) {
        ...
        if (!trimmed) continue;
        let event = null;
        try { event = JSON.parse(trimmed); } catch { /* non-JSON: leave event null */ }
    ...
    child.once("close", (code) => {
      ...
      store.transition(
        ...
        accepted ? "verifying" : scope.allowed ? "failed-retryable" : "failed-scope",
        {
          ...
          scope,
          commit: committedRevision(prepared, scope),
          logFile: built.logFile
        }
      );
...
export function runIssue(settings, issue, execute, runtime = { spawnSync, spawn }) {
  ...
  if (issue.canonicalState === "rework") {
    const prevRun = store.listRunsDetailed(100).find(r => r.issue_key === issue.key && r.state === "failed-retryable" && r.latest_payload?.reviewOutcome);
    if (prevRun) {
      const p = prevRun.latest_payload || {};
      promptLines.push(
        "--- REWORK REQUIRED ---",
        "This is a rework attempt for a previously failed review.",
        "Previous Implementation SHA: " + (p.reviewOutcome?.implementationSha || "unknown"),
        "Review Findings:\n" + (p.reviewOutcome?.evidence || []).join("\n"),
        "Previous Verification Scope (Files changed):\n" + JSON.stringify(p.scope?.changedFiles || [])
      );
    }
  }
