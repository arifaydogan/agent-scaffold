...
export function createConfigSnapshot(settings, issue, plan = {}) {
  ...
  const executionProfile = settings?.data?.executor
    ...
    : null;
  const reviewProfile = selectReviewProfile(settings, issue, plan);

  const persona = plan?.persona || "backend-engineer";
  const taskAgent = plan?.taskAgent || plan?.persona || "backend-engineer";
  const reviewPersona = reviewProfile?.persona || "qa-engineer";
  const reviewTaskAgent = reviewProfile?.taskAgent || reviewProfile?.persona || "qa-engineer";
  ...
  return {
    ...
    maxConcurrency: settings?.data?.policy?.maxConcurrency || 1,
...
export function issuePlan(settings, issue, options = {}) {
  ...
  if (snapshot?.allowedPaths !== undefined) {
    allowedPaths = snapshot.allowedPaths;
  } else if (configuredPathScopes === undefined || configuredPathScopes === null) {
    allowedPaths = Array.isArray(route.allowedPaths) ? route.allowedPaths : [];
  } else {
    const agentHardScope = configuredPathScopes[taskAgent];
    if (!Array.isArray(agentHardScope) || agentHardScope.length === 0) {
      allowedPaths = [];
    } else if (!Array.isArray(route.allowedPaths) || route.allowedPaths.length === 0) {
      allowedPaths = [];
    } else {
      allowedPaths = intersectPathScopes(route.allowedPaths, agentHardScope);
    }
  }
  ...
  if (settings.data?.executor) {
    if (snapshot?.executorProvider) {
      ...
      execution = selectExecutionProfile(settings, issue, route);
    }
  }
  ...
  const configSnapshot = snapshot || createConfigSnapshot(settings, issue, rawPlan);
          ...
          try {
            ...
            if (parsed.model || parsed.selected_model) {
              ...
              store.transition(runId, "model_selected", {
                provider: profile.provider,
                model: parsed.model || parsed.selected_model || profile.model,
                pid,
                workerLeaseId,
                workerLeaseExpiresAt: leaseExpiry()
              });
...
export function handleReview(settings, issue, execute, runtime = { spawnSync, spawn }) {
  ...
  const plan = issuePlan(settings, issue, { store, action: "review", originatingRun: queuedRun });
  ...
  if (profile.provider === "antigravity") {
    return spawnProviderAsync(
      { store, runId, built, profile, plan, prepared, issueKey: issue.key, timeoutMs: (profile.config?.timeoutSeconds || 3600) * 1000, workerLeaseSeconds: settings.data.supervisor?.staleAfterSeconds || 90, heartbeatMs: (settings.data.supervisor?.heartbeatSeconds || 10) * 1000, successState: "completed" },
      runtime
  ...
  if (telemetry.ok && returnCode === 0) {
    store.transition(runId, "completed", { returnCode, provider: profile.provider, scope });
  }
  return handleReviewCompletion({ exitCode: telemetry.ok && returnCode === 0 ? 0 : returnCode || 4, telemetry, scope });
}
