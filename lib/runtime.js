...
export function issuePlan(settings, issue, options = {}) {
  const eligibility = evaluateIssue(issue, settings.data.policy, {
    ...
    action: options.action || (issue.canonicalState === "review" ? "review" : "implementation")
  });
  const route = createOrchestratorProvider(settings).plan(issue);
  ...
  const configSnapshot = createConfigSnapshot(settings, issue, basePlan);
  return {
    ...basePlan,
    configSnapshot
  };
...
export function handleImplementation(settings, issue, execute, runtime = { spawnSync, spawn }, planOverrides = {}) {
  const store = getStore(settings);
  const basePlan = issuePlan(settings, issue, { store, approved: planOverrides.approved });
