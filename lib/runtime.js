...
export function spawnProviderAsync(params, runtime = { spawn }) {
  ...
  return new Promise((resolve) => {
    ...
    child.once("close", (code) => {
      ...
      const operatorRequest = scope.allowed
      ...
      if (typeof store.recordTelemetryEvent === "function") {
        store.recordTelemetryEvent({
          ...
          usage: normUsage,
          error: operatorRequest || accepted ? null : { category: scope.allowed ? "provider_process_error" : "scope_violation", safeMessage: `Process exited with code ${returnCode}` }
        });
      }

      if (operatorRequest) store.releaseLock(issueKey, runId);
      resolve({ exitCode: operatorRequest ? 2 : accepted ? 0 : scope.allowed ? returnCode || 4 : 5, telemetry, scope, operatorRequest });
    });
  });
}
...
export function runIssue(settings, issue, execute, runtime = { spawnSync, spawn }, options = {}) {
  ...
  if (state === "ready" || state === "in_progress" || state === "in-progress") {
    return handleImplementation(settings, issue, execute, runtime, options);
  }
  switch (state) {
    case "review":
