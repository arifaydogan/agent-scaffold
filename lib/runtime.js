...
export function createConfigSnapshot(settings, issue, plan = {}) {
  ...
  const orchestratorProvider = selectedOrchestratorProviderName(settings);
  ...
  return {
    ...
    autonomy,
    agentId: plan?.agent || plan?.persona || "backend-engineer",
    persona: plan?.persona || "backend-engineer",
    taskAgent: plan?.taskAgent || plan?.persona || "backend-engineer",
    skills: plan?.skills || [],
...
export function issuePlan(settings, issue, options = {}) {
  ...
  const route = snapshot
    ...
    : createOrchestratorProvider(settings).plan(issue);

  const allowedPaths = snapshot?.allowedPaths || settings.data?.policy?.pathScopes?.[route.persona] || [];
  ...
  const rawPlan = {
    ...
    orchestratorProvider: selectedOrchestratorProviderName(settings),
    allowedPaths,
    execution,
    ...worktree
  };
