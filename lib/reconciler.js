  ...
  for (const epic of store.listEpics(50)) {
    ...
    for (const integration of epic.integrations) {
      ...
      if (integration.state === "integrating" && review) {
        ...
        if (headRes.ok && sc.isAncestor(review.sha, headRes.sha, { repoPath: integrationWorktree })) {
        ...
        if (typeof store.recordTelemetryEvent === "function") {
          try {
            store.recordTelemetryEvent({
              eventId: `telem-conflict-${epic.key}-${integration.issueKey}-1`,
      ...
      if (typeof store.recordTelemetryEvent === "function") {
        try {
          store.recordTelemetryEvent({
            eventId: `telem-integrated-${epic.key}-${integration.issueKey}-1`,
            runId: review.runId || `int-${integration.issueKey}`,
            issueKey: integration.issueKey,
            role: "integration worker",
            action: "childIntegration",
            ...
            status: "completed",
            sequence: 999,
            metadata: { reviewedSha: review.sha, integratedSha }
          });
        } catch {}
      }
