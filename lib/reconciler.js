...
async function safeWorkSourceMutate(workSource, settings, action, ...args) {
  if (typeof workSource !== "object" || workSource === null) return;
  const policy = settings?.data?.policy || {};
  if (workSource.writeEnabled !== true) {
    throw new Error(`Work source provider ${workSource.providerName || 'unknown'} has writeEnabled=false`);
  }
  if (!policy.externalWritesEnabled) {
    throw new Error("policy.externalWritesEnabled is false or undefined");
  }
  if (policy.autonomyEnabled !== true) {
    throw new Error("policy.autonomyEnabled is false or undefined");
  }
  return await workSource[action](...args);
}
...
export function recordReviewerOutcome(
  ...
  if (!Array.isArray(evidence) || evidence.length === 0 || evidence.some((item) => !String(item).trim())) {
...
export function reconcileReviewers(settings, store, options = {}) {
  ...
  for (const run of runs) {
    if (run.state === "verifying") {
      ...
      const bypass = settings?.data?.policy?.reviewBypass || (run.latest_payload?.labels || []).includes("bypass-review");
      
      const nextState = bypass ? "reviewed-clean" : "review-queued";
      const payload = bypass
        ? {
            reviewOutcome: {
              implementationSha: sha,
              verdict: "clean",
              evidence: ["Bypassed via policy"],
              reviewerId: "system"
            }
          }
        : {
      ...
      if (success) {
        store.releaseLock(run.issue_key, run.id);
        if (!bypass) reviewsRequested += 1;
      }
    } else if (run.state === "review-failed") {
      const outcome = run.latest_payload?.reviewOutcome || null;
      const allRuns = store.listRunsDetailed(100);
      const reworkCount = allRuns.filter(r => r.issue_key === run.issue_key && r.payload?.attempt > 0).length;
      // We look at the actual attempt count if available, or fallback to zero
      const attempt = run.latest_payload?.attempt || 0;
      ...
      if (attempt >= maxReworkAttempts) {
        ...
        if (success) {
          ...
          if (options.execute && options.workSource) {
            ...
            const p1 = safeWorkSourceMutate(options.workSource, settings, "addComment", run.issue_key, comment);
            const p2 = safeWorkSourceMutate(options.workSource, settings, "transition", run.issue_key, "blocked", {});
            if (options.promises) {
              options.promises.push(p1, p2);
            }
          }
        }
      } else {
        ...
        if (success) {
          ...
          if (options.execute && options.workSource) {
             ...
             const p1 = safeWorkSourceMutate(options.workSource, settings, "addComment", run.issue_key, comment);
             const p2 = safeWorkSourceMutate(options.workSource, settings, "transition", run.issue_key, "rework", {});
             if (options.promises) {
               options.promises.push(p1, p2);
             }
          }
...
function acceptedReviews(runs) {
  ...
  for (const run of runs) {
    if (run.state !== "reviewed-clean" || accepted.has(run.issue_key)) continue;
...
export function reconcileIntegrations(settings, store, options = {}) {
  ...
  for (const epic of store.listEpics(50)) {
    for (const integration of epic.integrations) {
      ...
      if (!settings?.data?.policy?.externalWrites) {
        blocked.push({ epicKey: epic.key, issueKey: integration.issueKey, reason: "External writes disabled by policy" });
        continue;
      }
...
export function reconcileStandaloneReviews(settings, store, options = {}) {
  ...
  for (const [issueKey, review] of accepted.entries()) {
    ...
    // Transition the run to "completed" so it stops getting processed
    const success = compareAndSet(store, review.runId, "reviewed-clean", "completed", { reason: "Standalone review completed" }, atMs);
    if (success) {
      transitioned += 1;
      if (options.execute && options.workSource) {
        const comment = "## Review Completed\nClean review verdict reached for standalone issue.";
        const p1 = safeWorkSourceMutate(options.workSource, settings, "addComment", issueKey, comment);
        const p2 = safeWorkSourceMutate(options.workSource, settings, "transition", issueKey, "human_approval", {});
        if (options.promises) options.promises.push(p1, p2);
      }
    }
  }
  return { transitioned };
...
export function tick(settings, store, options = {}) {
  const atMs = nowMs(options);
  const workers = reconcileWorkers(settings, store, { now: atMs });
  const reviewers = reconcileReviewers(settings, store, { now: atMs });
