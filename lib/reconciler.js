...
export function reconcileIntegrations(settings, store, options = {}) {
  ...
  const integrationAdapter = options.integrationAdapter || (options.execute ? ((req) => {
    ...
    return sc.integrateReviewedRevision(settings, req);
  }) : null);
  ...
  if (typeof store.listParentExecutions === "function" && typeof store.listEpicTasks === "function") {
    ...
    for (const parent of parentExecutions) {
      ...
      for (const task of tasks) {
        ...
        // Check if already queued, claimed, or integrated
        const alreadyQueued = existingIntegrations.some(
          (i) => i.issueKey === task.issueKey && ["queued", "claimed", "integrated"].includes(i.state)
        );
        ...
        recordParentTelemetryEvent(store, {
          ...
          stage: "progress",
          status: "running",
          payload: { issueKey: task.issueKey, reviewedSha: review.sha }
        });
        recordParentTelemetryEvent(store, {
          parentKey: parent.parentKey,
          ...
          stage: "progress",
          status: "running",
          payload: { issueKey: task.issueKey, leafBranch: task.branch, targetBranch: parent.integrationBranch }
        });
      }
    }
  }
  ...
  for (const epic of store.listEpics(50)) {
    for (const integration of epic.integrations) {
        ...
        if (typeof store.recordTelemetryEvent === "function") {
          try {
            store.recordTelemetryEvent({
              eventId: `telem-conflict-${epic.key}-${integration.issueKey}-${Date.now()}`,
              runId: review.runId || `int-${integration.issueKey}`,
              issueKey: integration.issueKey,
              ...
              status: "failed",
              sequence: 999,
              error: { category: "integration_conflict", safeMessage: reason }
            });
          } catch {}
        }
      ...
      if (typeof store.recordTelemetryEvent === "function") {
        try {
          store.recordTelemetryEvent({
            eventId: `telem-integrated-${epic.key}-${integration.issueKey}-${Date.now()}`,
            runId: review.runId || `int-${integration.issueKey}`,
            issueKey: integration.issueKey,
            role: "integration worker",
            action: "childIntegration",
            ...
            status: "completed",
            sequence: 999,
            rawPayload: { reviewedSha: review.sha, integratedSha }
          });
        } catch {}
      }
