...
export function reconcileReviewers(settings, store, options = {}) {
  ...
  for (const run of runs) {
    if (run.state === "verifying") {
      ...
      const nextState = bypass ? "reviewed-clean" : "transitioning-review";
      ...
      if (success) {
        if (bypass) store.releaseLock(run.issue_key, run.id);
      ...
      if (options.execute && options.workSource) {
        const p = (async () => {
          try {
            await safeWorkSourceMutate(options.workSource, settings, "transition", run.issue_key, "review", {});
