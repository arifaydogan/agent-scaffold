...
export function reconcileReviewers(settings, store, options = {}) {
  ...
  for (const run of runs) {
    if (run.state === "verifying") {
      ...
      if (success && !bypass) reviewsRequested += 1;
      ...
      const reworkCount = allRuns.filter(r => r.issue_key === run.issue_key && (r.state === "review-failed" || (r.state === "failed-retryable" && r.latest_payload?.reviewOutcome))).length;
      ...
      if (reworkCount >= maxReworkAttempts) {
        ...
        if (success) {
          ...
          if (options.execute && options.workSource) {
            const comment = `## Human Attention Required\nAgent rework limit exhausted. Previous finding: ${outcome?.verdict}`;
            const p1 = options.workSource.addComment(run.issue_key, comment).catch(() => {});
            const p2 = options.workSource.transition(run.issue_key, "blocked", {}).catch(() => {});
            if (options.promises) {
        ...
        if (success) {
          ...
          if (options.execute && options.workSource) {
             ...
             const p1 = options.workSource.addComment(run.issue_key, comment).catch(() => {});
             const p2 = options.workSource.transition(run.issue_key, "rework", {}).catch(() => {});
             if (options.promises) {
               options.promises.push(p1, p2);
             }
          }
