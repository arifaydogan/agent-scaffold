...
export function createDashboardServer(settings, options = {}) {
  ...
  return http.createServer(async (request, response) => {
    ...
    // ── Phase H: Parent Orchestration Inspection API ─────────────────────────
    if (url.pathname.startsWith("/api/parent/") || url.pathname.startsWith("/api/pm/parents/")) {
      if (!["GET", "HEAD"].includes(method)) {
        send(response, 405, "Method not allowed", "text/plain; charset=utf-8", method);
        return;
      }
      const parentKey = decodeURIComponent(url.pathname.replace(/^\/api\/(?:parent|pm\/parents)\/?/, ""));
