...
export function buildDashboardSnapshot(settings, options = {}) {
  ...
  return {
    ...
    capabilities: { retryHandler: options.retryHandlerAvailable === true },
    supervisor,
  ...
  return http.createServer(async (request, response) => {
    ...
    if (url.pathname === "/api/snapshot") {
      const snapshot = options.demo
        ...
        : buildDashboardSnapshot(settings, { store, retryHandlerAvailable: typeof options.retryHandler === "function" });
