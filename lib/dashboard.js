  ...
  return http.createServer(async (request, response) => {
    ...
    if (url.pathname === "/api/pm/work-items" || url.pathname.startsWith("/api/pm/work-items/")) {
      ...
      if (parts.length === 2 && parts[1] === "approve" && method === "POST") {
        ...
        const isPmMutationAllowed = activeSettings.data.controlPlane?.pmMutationEnabled !== undefined
          ? Boolean(activeSettings.data.controlPlane.pmMutationEnabled)
          : (activeSettings.data.controlPlane?.configMutationEnabled !== undefined
              ? Boolean(activeSettings.data.controlPlane.configMutationEnabled)
              : true);
