...
export function createDashboardServer(settings, options = {}) {
  ...
  const executionPlanFor = async issueKey => {
    let rawPlan;
    if (typeof options.planHandler === "function") {
      rawPlan = await options.planHandler({ issueKey, settings: activeSettings, store });
    } else {
      const workSource = createDashboardWorkSource();
      let issue = typeof workSource?.getWorkItem === "function"
      ...
      rawPlan = issuePlan(activeSettings, issue, { store });
    }
    ...
    return { rawPlan: { ...rawPlan, planFingerprint: plan.planFingerprint }, plan };
  };
  return http.createServer(async (request, response) => {
    ...
    if (executionAction) {
      ...
      try {
        ...
        const allowedFields = action === "plan" ? [] : ["planFingerprint"];
        if (Object.keys(body).some(key => !allowedFields.includes(key))) {
          throw new Error("Unsupported execution field");
        }
        const planned = await executionPlanFor(issueKey);
        if (action === "plan") {
          send(response, 200, JSON.stringify({ ok: true, plan: planned.plan }), "application/json; charset=utf-8", method);
          return;
        }
        ...
        send(response, error.statusCode || 400, JSON.stringify({ error: error.message }), "application/json; charset=utf-8", method);
      }
      return;
    }
