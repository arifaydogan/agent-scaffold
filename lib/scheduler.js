...
export function selectDispatchBatch(
  ...
  for (const candidate of candidates) {
    ...
    if (agentId && Object.prototype.hasOwnProperty.call(agentConcurrency, agentId)) {
      agentLimit = Math.min(agentLimit, Math.max(0, agentConcurrency[agentId]));
    }
  ...
  // Check exclusive (non-parallel-safe) tasks that have their dependencies resolved.
  // Must also satisfy provider and agent concurrency constraints.
  ...
  for (const candidate of exclusiveCandidates) {
    ...
    if (agentId && agentLimit < 1) continue;
