...
import { computePlanFingerprint, computeIntegrationFingerprint } from "./policy.js";
...
export class RunStore {
    ...
    try {
      ...
      if (agentCols.length > 0 && !agentCols.includes("current_version")) {
        ...
        for (const row of legacyRows) {
          let defObj = {};
          try { defObj = JSON.parse(row.definition); } catch {}
        ...
        this.database.exec("DROP TABLE agent_definitions_legacy;");
      }
    } catch {}
  }
  ...
  hasExecutionApproval(issueKey, { action, plan = null, planFingerprint = null, attempt = 0 } = {}) {
    ...
    if (!expectedFingerprint && plan) {
      expectedFingerprint = action === "childIntegration"
        ? computeIntegrationFingerprint(plan)
  ...
  updateAgentDefinition(id, partialOrFullDefinition) {
    ...
    const merged = { ...existing.definition, ...partialOrFullDefinition, id };
    ...
    this.database.prepare(
      "UPDATE agent_definitions SET status = ?, current_version = ?, updated_at = ? WHERE id = ?"
    ).run(validated.status || "enabled", nextVersion, now, id);
  ...
  getAgentDefinition(id, version = null) {
    ...
    return {
      ...
      definition: { ...definition, id: defRow.id, status: defRow.status },
  ...
  isAgentUsed(id) {
    ...
    const row = this.database.prepare(
      "SELECT 1 FROM runs WHERE payload LIKE '%\"taskAgent\":\"' || ? || '\"%' OR payload LIKE '%\"agentId\":\"' || ? || '\"%' OR payload LIKE '%\"agent\":\"' || ? || '\"%' LIMIT 1"
  ...
  seedBuiltinAgents() {
    const count = this.database.prepare("SELECT COUNT(*) as cnt FROM agent_definitions").get()?.cnt || 0;
    if (count === 0) {
