...
export class RunStore {
  ...
  listUsageEvents(runId = null) {
    if (runId) {
      return this.database.prepare(
        "SELECT * FROM usage_events WHERE run_id = ? ORDER BY id ASC"
      ).all(runId).map(row => ({
        id: row.id,
        runId: row.run_id,
    ...
    return this.database.prepare(
      "SELECT * FROM usage_events ORDER BY id DESC LIMIT 200"
    ).all().map(row => ({
      id: row.id,
      runId: row.run_id,
      provider: row.provider,
  ...
  recordTelemetryEvent(data = {}) {
    ...
    try {
      ...
      if (usage.available && (usage.inputTokens || usage.outputTokens)) {
        this.database.prepare(`
          INSERT INTO usage_events(run_id, provider, model, input_tokens, output_tokens, duration_ms, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(
  ...
  addUsageEvent(runId, provider, model, inputTokens, outputTokens, durationMs) {
    ...
    ).run(runId, provider, model, inputTokens != null ? inputTokens : null, outputTokens != null ? outputTokens : null, durationMs != null ? durationMs : null, now);
  }

  recordUsageEvent(...args) {
    if (args.length === 1 && typeof args[0] === "object" && args[0] !== null) {
      const { runId, provider, model, inputTokens, outputTokens, durationMs } = args[0];
      return this.addUsageEvent(runId, provider, model, inputTokens, outputTokens, durationMs);
    }
    return this.addUsageEvent(...args);
  }
