...
export class RunStore {
  ...
  recordTelemetryEvent(data = {}) {
    const eventId = String(data.eventId || `${data.runId}-${Date.now()}-${Math.random()}`);
    ...
    `).run(
      ...
      usage.available ? 1 : 0,
      data.error?.category ? String(data.error.category) : null,
      data.error?.safeMessage ? String(data.error.safeMessage) : null,
      data.raw ? JSON.stringify(data.raw) : null,
      now
    );

    if (usage.available && data.runId && (usage.inputTokens || usage.outputTokens)) {
