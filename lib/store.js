...
export class RunStore {
  constructor(databasePath) {
    ...
    if (parentExecCols.length > 0) {
      ...
      if (!parentExecCols.includes("reviewer_snapshot_json")) this.database.exec("ALTER TABLE parent_executions ADD COLUMN reviewer_snapshot_json TEXT;");
    }
  ...
  withTransaction(fn) {
    if (this._txDepth === undefined) this._txDepth = 0;
    const isOuter = this._txDepth === 0;
    if (isOuter) {
      let attempts = 0;
      while (true) {
        try {
          ...
          if ((err.code === "ERR_SQLITE_ERROR" || err.message?.includes("busy") || err.message?.includes("locked")) && attempts < 50) {
            attempts++;
  ...
  createRunAndClaimIssue(issueKey, payload) {
    const id = randomUUID();
    const now = new Date().toISOString();
    const body = JSON.stringify(payload);
    return this.withTransaction(() => {
      ...
      this.database
        .prepare("INSERT INTO runs VALUES (?, ?, ?, ?, ?, ?)")
        .run(id, issueKey, "claimed", body, now, now);
      this.#event(id, "discovered", payload, now);
      this.#event(id, "eligible", payload, now);
      this.#event(id, "claimed", payload, now);
      return { claimed: true, runId: id, run: this.getRun(id) };
    });
  }
  ...
  upsertParentExecution({
    ...
    const reviewerJson = (reviewerSnapshot || configSnapshot) ? JSON.stringify(reviewerSnapshot || configSnapshot) : null;
    this.database.prepare(
      `INSERT INTO parent_executions(
         ...
         graph_fingerprint, dag_json, state, completion_packet_json, drift_detected, reviewer_snapshot_json, created_at, updated_at
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(parent_key) DO UPDATE SET
         ...
         reviewer_snapshot_json = coalesce(excluded.reviewer_snapshot_json, parent_executions.reviewer_snapshot_json),
         updated_at = excluded.updated_at`
    ).run(
      parentKey,
      sourceProvider,
  ...
  #shapeParentExecution(row) {
    ...
    let reviewerSnapshot = null;
    if (row.reviewer_snapshot_json) {
      try { reviewerSnapshot = JSON.parse(row.reviewer_snapshot_json); } catch {}
    }
    return {
      ...
      completionPacket,
      reviewerSnapshot,
      configSnapshot: reviewerSnapshot,
